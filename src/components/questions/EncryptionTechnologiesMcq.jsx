...
const rawQuestions = [
  {
    id: 1,
    question: "A company deploys new Windows laptops and configures BitLocker full-disk encryption. BitLocker binds the encryption keys to a specialized cryptographic hardware chip soldered onto the motherboard that stores hardware configuration measurements and defends against dictionary attacks. What hardware component is used?",
    options: [
      { 
      ...
      { 
        text: "SQL Injection", 
        isCorrect: false, 
        whyWrong: "SQL Injection is a web database attack technique, not a hardware chip." 
      },
      { 
        text: "Typosquatting", 
        isCorrect: false, 
    ...
    laymanExplanation: "A Trusted Platform Module (TPM) is a dedicated security chip built directly into your computer motherboard! It securely stores encryption keys (like BitLocker keys) and verifies that system boot files haven't been tampered with."
  },
  {
    id: 2,
    question: "A Certificate Authority (CA) needs a high-end enterprise hardware device with redundant power supply units to securely store thousands of root private keys and offload heavy SSL/TLS cryptographic processing from web servers. What dedicated hardware appliance is installed in the data center rack?",
    options: [
      { 
    ...
    laymanExplanation: "A Hardware Security Module (HSM) is a heavy-duty enterprise vault! Used in high-security data centers, an HSM securely generates, backs up, and manages thousands of master encryption keys while offloading crypto math from servers."
  },
  {
    id: 3,
    question: "A cloud security architect configures a centralized platform to manage the lifecycle of all corporate encryption keys. The platform automatically rotates keys every 90 days, logs all key access events, and ensures encryption keys are separated from the database servers storing the encrypted files. What system is implemented?",
    options: [
      { 
    ...
    laymanExplanation: "A Key Management System (KMS) is a central control dashboard for keys! It handles creating keys, assigning them to cloud services, rotating them on a schedule, and keeping keys far away from stored data so a database breach doesn't expose the keys."
  },
  {
    id: 4,
    question: "A smartphone manufacturer designs a hardware-isolated processing area inside the phone's CPU. This area runs its own boot ROM, performs real-time memory encryption, and securely processes facial recognition biometric templates so that even if the main smartphone OS kernel is compromised, the biometric data remains safe. What feature is described?",
    options: [
      { 
        text: "Secure Enclave (Hardware Co-processor Isolation)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "A Secure Enclave is a secret vault inside your CPU chip! It runs completely separate from the main operating system, protecting sensitive biometrics (like Face ID or fingerprints) even if malware takes over the phone OS."
  },
  {
    id: 5,
    question: "Why do enterprise security guidelines strictly forbid storing cryptographic keys on the exact same database server or hard drive volume where encrypted customer records are saved?",
    options: [
      { 
        text: "If an attacker compromises the storage volume, having keys stored alongside the data allows instant decryption of all stolen records", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not a security separation rule." 
      },
      { 
    ...
    laymanExplanation: "Never leave the key under the doormat! Storing keys on the same server as the encrypted data means a hacker who steals the database gets the key for free. KMS separates keys from data!"
  },
  {
    id: 6,
    question: "An IT auditor reviews physical security for a corporate laptop fleet versus an enterprise payment processing cluster. Which statement accurately compares TPM and HSM hardware?",
    options: [
      { 
        text: "TPM is a motherboard chip designed for single devices, while HSM is a high-throughput enterprise appliance for managing thousands of keys", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "TPM is for individual PCs/laptops (personal key chip), while HSM is a giant server appliance built for data centers (managing thousands of keys and accelerating corporate SSL traffic)."
  }
];
...
export default function EncryptionTechnologiesMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-amber-600 via-yellow-600 to-amber-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            1.4 - Encryption Technologies Practice 🛡️💻
          </h1>
          <p class="text-amber-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Trusted Platform Module (TPM), Hardware Security Module (HSM), Key Management Systems (KMS), Secure Enclaves, and Key Separation!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 1.4 Encryption Technologies for CompTIA Security+!'
