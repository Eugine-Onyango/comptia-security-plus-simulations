...
const rawQuestions = [
  {
    id: 1,
    question: "A Linux administrator downloads a disk image ISO and compares its local SHA-256 checksum against the hash value published on the official software website. Both hash strings match perfectly. What security goal is verified?",
    options: [
      { 
        text: "Data Integrity (confirming the file was not altered or corrupted)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        text: "SQL Injection", 
        isCorrect: false, 
        whyWrong: "SQL Injection is a web database attack technique, not a file verification property." 
      },
      { 
        text: "Typosquatting", 
        isCorrect: false, 
    ...
    laymanExplanation: "Hashes prove Integrity! If even a single byte of a file changes, the entire hash digest changes completely. Matching hashes prove the file wasn't corrupted or tampered with."
  },
  {
    id: 2,
    question: "A security audit requires an organization to replace MD5 hashing in all authentication systems because attackers can craft two completely different input files that generate the exact same MD5 digest output. What cryptographic vulnerability is cited?",
    options: [
      { 
        text: "Hash Collision", 
        isCorrect: true, 
    ...
    laymanExplanation: "A Hash Collision happens when two different files produce the EXACT SAME hash! MD5 is obsolete because hackers found easy ways to forge matching MD5 hashes."
  },
  {
    id: 3,
    question: "To protect stored user passwords, a system appends a unique random string of characters to each password prior to hashing. As a result, two users who choose the password 'dragon' end up with completely different stored hash values, neutralizing precomputed lookup table attacks. What technique is used?",
    options: [
      { 
        text: "Salting the Hash (defeating Rainbow Table attacks)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Salting adds a pinch of random secret sauce to every password! Adding a unique random string to each password before hashing ensures precomputed Rainbow Tables can't crack the passwords."
  },
  {
    id: 4,
    question: "Alice wants to publish a software update with a digital signature so customers can confirm the file originated from her and was not modified by a third party. Which key must Alice use to encrypt the message digest and create the digital signature?",
    options: [
      { 
    ...
    laymanExplanation: "To CREATE a Digital Signature, sign with your PRIVATE KEY! Because only Alice possesses her private key, nobody else could have created that signature."
  },
  {
    id: 5,
    question: "A client workstation downloads a digitally signed executable from Alice. To verify that the digital signature is valid and authentic, which key must the client's operating system use to decrypt the digital signature?",
    options: [
      { 
      ...
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not a signature verification key." 
      },
      { 
    ...
    laymanExplanation: "To VERIFY a Digital Signature, decrypt it with the sender's PUBLIC KEY! If Alice's public key successfully decrypts the signature, you have 100% proof it came from Alice."
  },
  {
    id: 6,
    question: "A digitally signed executive contract guarantees Integrity (the contract wasn't altered) and Authentication (identifies the signer). What third core security property ensures the executive cannot later deny signing the contract?",
    options: [
      { 
    ...
    laymanExplanation: "Non-repudiation means 'No take-backs!' Because the digital signature could only be created using the signer's unique private key, they cannot claim 'I didn't sign that!'"
  }
];
...
export default function HashingSignaturesMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-amber-600 via-yellow-600 to-amber-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            1.4 - Hashing & Digital Signatures Practice 🔏
          </h1>
          <p class="text-amber-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Hashes (SHA-256), Hash Collisions (MD5), Salting Hashes (defeating Rainbow Tables), and Digital Signatures (Sign with Private, Verify with Public)!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 1.4 Hashing & Digital Signatures for CompTIA Security+!'
