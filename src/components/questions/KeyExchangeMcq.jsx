...
const rawQuestions = [
  {
    id: 1,
    question: "A network administrator needs to set up an IPsec VPN tunnel with a third-party vendor. To ensure the pre-shared secret key is not intercepted on the network, the administrator calls the vendor over a secure phone line and reads the secret key aloud. What key exchange method is used?",
    options: [
      { 
        text: "Out-of-Band Key Exchange", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        text: "SQL Injection", 
        isCorrect: false, 
        whyWrong: "SQL Injection is a web database attack technique, not a key exchange method." 
      },
      { 
        text: "Typosquatting", 
        isCorrect: false, 
    ...
    laymanExplanation: "Out-of-Band Key Exchange means sharing the key OUTSIDE the main network channel! Using a phone call, physical courier, or in-person handshake ensures the secret key never travels across the Internet where hackers might sniff it."
  },
  {
    id: 2,
    question: "A web browser establishes an encrypted HTTPS session with a bank website. The browser generates a random symmetric session key, encrypts it using the bank's public key, and transmits it directly over the network connection. What key exchange method is used?",
    options: [
      { 
        text: "In-Band Key Exchange", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "In-Band Key Exchange sends the key right over the network channel, but wraps it inside asymmetric encryption so snoopers can't steal it!"
  },
  {
    id: 3,
    question: "Bob combines his private key with Alice's public key, while Alice combines her private key with Bob's public key. Through mathematical algorithms, both parties derive the exact same shared symmetric key without ever sending the symmetric key across the network. What key exchange mechanism is depicted?",
    options: [
      { 
        text: "Diffie-Hellman (DH) Key Exchange (Symmetric key derived from asymmetric keys)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Diffie-Hellman Key Exchange is mathematical magic! Bob uses (Bob's Private + Alice's Public) and Alice uses (Alice's Private + Bob's Public). Both end up with the EXACT SAME secret key without ever broadcasting it over the wire!"
  },
  {
    id: 4,
    question: "A security protocol generates temporary, short-lived session keys for every web transaction. Because these session keys are unpredictable and discarded after each session, compromising a server's long-term private key in the future cannot decrypt past recorded sessions. What property is provided?",
    options: [
      { 
        text: "Ephemeral Keys / Perfect Forward Secrecy (PFS)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Ephemeral Keys are disposable keys! They live for just one session and are thrown away. Even if a hacker steals the server's master key 5 years later, they still can't decrypt past conversations."
  },
  {
    id: 5,
    question: "Why do modern secure communications protocols (like TLS and SSH) use asymmetric encryption strictly during the initial key exchange handshake to negotiate a symmetric session key for bulk data transfer?",
    options: [
      { 
        text: "Symmetric encryption is significantly faster and less computationally expensive for bulk data transfer", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not a performance rationale for session keys." 
      },
      { 
    ...
    laymanExplanation: "Speed! Asymmetric math is slow and heavy. Once the two computers use asymmetric keys to safely exchange a secret symmetric key, they switch to high-speed symmetric encryption to handle fast streaming and browsing."
  },
  {
    id: 6,
    question: "In a Diffie-Hellman asymmetric key exchange, Alice needs to compute the shared symmetric session key on her computer. Which two key inputs does Alice combine?",
    options: [
      { 
        text: "Alice's Private Key and Bob's Public Key", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Alice combines HER PRIVATE KEY with BOB'S PUBLIC KEY! Meanwhile, Bob combines HIS PRIVATE KEY with ALICE'S PUBLIC KEY. Thanks to math properties, both calculations yield identical secret keys."
  }
];
...
export default function KeyExchangeMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-amber-600 via-yellow-600 to-amber-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            1.4 - Key Exchange Practice 🔄🔑
          </h1>
          <p class="text-amber-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Out-of-Band vs In-Band Key Exchange, Diffie-Hellman (DH) Key Agreement, Ephemeral Keys, Perfect Forward Secrecy (PFS), and Symmetric Session Keys!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 1.4 Key Exchange for CompTIA Security+!'
