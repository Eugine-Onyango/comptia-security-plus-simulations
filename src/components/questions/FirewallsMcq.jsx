...
const rawQuestions = [
  {
    id: 1,
    question: "A firewall ACL explicitly allows web traffic on TCP port 80 and HTTPS on port 443. When a remote user attempts an FTP connection on TCP port 21, the firewall automatically blocks it despite having no rule mentioning port 21. What default firewall rule dropped this traffic?",
    options: [
      { 
        text: "Implicit Deny", 
        isCorrect: true, 
    ...
    laymanExplanation: "Implicit Deny is the bouncer at the end of the line! Unless a firewall rule explicitly says 'ALLOW', the final unwritten rule at the bottom of the ACL drops all remaining traffic by default."
  },
  {
    id: 2,
    question: "An attacker attempts to bypass network filters by disguising BitTorrent peer-to-peer file sharing inside HTTPS packets on port 443. A firewall analyzes the Layer 7 application payload, recognizes BitTorrent signatures, and blocks the connection. What firewall type was used?",
    options: [
      { 
        text: "Next-Generation Firewall (NGFW) with Deep Packet Inspection", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "NGFWs look inside the packet envelope! Unlike old firewalls that only check port numbers, a Next-Generation Firewall (NGFW) uses Layer 7 Deep Packet Inspection (DPI) to identify what application is actually running."
  },
  {
    id: 3,
    question: "An e-commerce firm places public web servers on a separate network segment isolated between the public Internet and the internal corporate LAN, preventing compromised web servers from accessing internal databases. What network zone was deployed?",
    options: [
      { 
        text: "Screened Subnet (DMZ)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "A Screened Subnet (DMZ) is a buffer zone! Public-facing servers live in this middle ground so if hackers compromise a web server, they are still blocked from reaching internal private servers."
  },
  {
    id: 4,
    question: "An integrated Intrusion Prevention System (IPS) flags a brand-new zero-day attack payload by detecting that incoming database queries deviate significantly from the server's established normal traffic baseline. What detection mechanism identified the threat?",
    options: [
      { 
        text: "Anomaly-Based Detection Engine", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Anomaly detection flags weird behavior! Anomaly-based IPS learns what normal network traffic looks like, so when weird, abnormal traffic arrives—even from a zero-day—it gets flagged."
  },
  {
    id: 5,
    question: "A junior admin appends a new rule allowing SSH (port 22) to the very bottom of a firewall ACL, beneath the default 'deny all' statement. Why are remote SSH connections still failing?",
    options: [
      { 
        text: "Firewalls process ACL rules top-to-bottom and match the 'deny all' rule first", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not an ACL rule processing issue." 
      },
      { 
        text: "Disabling WPA3 enterprise encryption", 
        isCorrect: false, 
        whyWrong: "Disabling WPA3 affects Wi-Fi security, not firewall ACL evaluation order." 
      },
      { 
        text: "Forcing CPU thermal throttling", 
        isCorrect: false, 
        whyWrong: "Thermal throttling is a hardware heat response, not an ACL rule evaluation order issue." 
      }
    ],
    laymanExplanation: "Firewall rules run top-to-bottom! Once traffic matches a rule (like 'deny all'), the firewall stops looking. Specific ALLOW rules must always be placed ABOVE general DENY rules."
  },
  {
    id: 6,
    question: "A network engineer configures a firewall rule to allow Windows Remote Desktop Protocol (RDP) management sessions from an admin subnet. Which destination port and transport protocol must be permitted?",
    options: [
      { 
        text: "TCP Port 3389", 
        isCorrect: true, 
    ...
    laymanExplanation: "RDP uses TCP Port 3389! Windows Remote Desktop sessions communicate over TCP 3389."
  }
];
...
export default function FirewallsMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-blue-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            4.5 - Firewalls Practice 🔥
          </h1>
          <p class="text-blue-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Implicit Deny, Next-Gen Firewalls (NGFW Layer 7 DPI), Screened Subnet (DMZ), Anomaly-based IPS, ACL Rule Evaluation Order, and Ports (RDP TCP/3389)!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-blue-50 border border-blue-200 rounded-2xl max-w-md mx-auto text-blue-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 4.5 Firewalls for CompTIA Security+!'
