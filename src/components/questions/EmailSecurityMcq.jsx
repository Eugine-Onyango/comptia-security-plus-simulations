...
const rawQuestions = [
  {
    id: 1,
    question: "To prevent attackers from sending fake emails that pretend to originate from a corporate domain, an administrator publishes a DNS TXT record that explicitly lists all authorized IP addresses allowed to send email for the company. What protocol was deployed?",
    options: [
      { 
    ...
    laymanExplanation: "SPF is an authorized sender guest list in DNS! It publishes a list of IP addresses that are allowed to send emails from your company domain. Recipient servers check this list to catch fake senders."
  },
  {
    id: 2,
    question: "An organization wants to guarantee that outbound emails are not modified or tampered with in transit. The outgoing mail server attaches a cryptographic digital signature to message headers, and publishes the public key in a DNS TXT record. What technology is used?",
    options: [
      { 
    ...
    laymanExplanation: "DKIM puts a wax seal digital signature on emails! The sending server signs the email with a private key, and the receiving server checks the signature using the public key in DNS to prove the email wasn't altered."
  },
  {
    id: 3,
    question: "A company has enabled SPF and DKIM. To dictate that receiving mail servers must automatically REJECT or QUARANTINE any incoming message that fails SPF/DKIM verification, what email policy framework is published in DNS?",
    options: [
      { 
    ...
    laymanExplanation: "DMARC gives instructions on what to do when SPF or DKIM fail! Domain owners use DMARC to tell receiving servers: 'If an email fails SPF or DKIM, reject it completely and send me a report!'"
  },
  {
    id: 4,
    question: "An enterprise deploys a security device in front of its mail servers to scan incoming emails for phishing links, malware attachments, and spam before delivering approved messages to employee inboxes. What appliance is being used?",
    options: [
      { 
        text: "Mail Gateway (Secure Email Gateway - SEG)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "A Mail Gateway acts like a security guard for email! It intercepts incoming emails, scans them for dangerous viruses or phishing links, and drops bad messages before employees open them."
  },
  {
    id: 5,
    question: "An email administrator receives automated aggregate XML reports detailing every IP address across the internet that sent emails using the company domain name and whether they passed authentication. What system generates these reports?",
    options: [
      { 
        text: "DMARC Compliance Reporting", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not DMARC reporting." 
      },
      { 
        text: "Disabling WPA3 enterprise encryption", 
        isCorrect: false, 
        whyWrong: "Disabling WPA3 affects Wi-Fi security, not DMARC email reporting." 
      },
      { 
        text: "Forcing CPU thermal throttling", 
        isCorrect: false, 
        whyWrong: "Thermal throttling is a hardware heat response, not DMARC email reporting." 
      }
    ],
    laymanExplanation: "DMARC reporting gives domain owners total visibility! Receiving servers email reports back to you showing who tried to send emails under your domain name."
  },
  {
    id: 6,
    question: "Why are standard, unconfigured SMTP mail servers inherently vulnerable to email spoofing attacks?",
    options: [
      { 
        text: "Default SMTP protocols do not verify whether the sender's address matches the actual sending IP", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Basic SMTP accepts any return address without checking! Old email protocols trust whatever address is written on the envelope, which is why SPF, DKIM, and DMARC are necessary to verify senders."
  }
];
...
export default function EmailSecurityMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-blue-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            4.5 - Email Security Practice 📧
          </h1>
          <p class="text-blue-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering SPF (Sender Policy Framework), DKIM Digital Signatures, DMARC Dispositions & Reports, and Mail Gateways!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-blue-50 border border-blue-200 rounded-2xl max-w-md mx-auto text-blue-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 4.5 Email Security for CompTIA Security+!'
