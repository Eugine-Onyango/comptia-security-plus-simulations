...
const rawQuestions = [
  {
    id: 1,
    question: "An enterprise deploys a web SSO solution that allows employees to log into a third-party cloud HR application using XML-based assertions sent between the corporate Identity Provider (IdP) and the cloud Service Provider (SP). What protocol is being used?",
    options: [
      { 
    ...
    laymanExplanation: "SAML uses XML assertions like a digital passport for web browsers! It allows your company (Identity Provider) to tell a web app (Service Provider) that you are authenticated without sharing your raw password."
  },
  {
    id: 2,
    question: "A mobile application developer wants to grant a photo-printing service access to photos stored in a user's cloud storage without requiring the user to share their account password. Which open standard authorization framework issues secure access tokens for API authorization?",
    options: [
      { 
        text: "OAuth 2.0", 
        isCorrect: true, 
    ...
    laymanExplanation: "OAuth is a valet key for data access! It grants an app permission (via tokens) to view specific files without giving away your master account password."
  },
  {
    id: 3,
    question: "An e-commerce website enables 'Log in with Google' on mobile devices, allowing users to authenticate seamlessly. Which authentication identity layer built on top of OAuth 2.0 handles this user authentication?",
    options: [
      { 
    ...
    laymanExplanation: "OpenID Connect (OIDC) adds user sign-in to OAuth! While OAuth handles permission tokens, OIDC handles proving WHO you are so you can click 'Log in with Google'."
  },
  {
    id: 4,
    question: "A network firewall and VPN concentrator query a central Active Directory server over IP to authenticate remote users and check group membership structured in an X.500 hierarchical directory tree. What directory access protocol is used?",
    options: [
      { 
    ...
    laymanExplanation: "LDAP is an electronic phonebook lookup for IT systems! VPN servers and firewalls use LDAP to look up users and passwords inside Active Directory."
  },
  {
    id: 5,
    question: "Company A and Company B set up a mutual trust relationship allowing Company A employees to log into Company B's B2B partner portal using their existing Company A credentials. What access management architecture is implemented?",
    options: [
      { 
        text: "Federated Identity Management (Federation)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not Federated IAM." 
      },
      { 
        text: "Disabling WPA3 enterprise encryption", 
        isCorrect: false, 
        whyWrong: "Disabling WPA3 affects Wi-Fi security, not Federated IAM." 
      },
      { 
        text: "Forcing CPU thermal throttling", 
        isCorrect: false, 
        whyWrong: "Thermal throttling is a hardware heat response, not Federated IAM." 
      }
    ],
    laymanExplanation: "Federation connects login systems between separate companies! It allows two partner businesses to trust each other's credentials without making employees create duplicate accounts."
  },
  {
    id: 6,
    question: "In an LDAP X.500 Directory Information Tree (DIT), an admin creates an entry for an individual user account, printer, or computer workstation. In the hierarchical tree structure, what is this individual resource object called?",
    options: [
      { 
        text: "Leaf Object", 
        isCorrect: true, 
    ...
    laymanExplanation: "In a directory tree, end items like users, PCs, and printers are Leaf Objects! Folders and organizational units (OUs) are branch containers, while actual items at the ends are leaves."
  }
];
...
export default function FederatedIamMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-blue-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            4.6 - Federated IAM & Protocols Practice 🌐
          </h1>
          <p class="text-blue-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering LDAP, SAML Web SSO, OAuth 2.0 Token Authorization, OpenID Connect (OIDC), Identity Federation, and X.500 Leaf Objects!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-blue-50 border border-blue-200 rounded-2xl max-w-md mx-auto text-blue-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 4.6 Federated IAM & Protocols for CompTIA Security+!'
