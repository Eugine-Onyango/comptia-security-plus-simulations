...
const rawQuestions = [
  {
    id: 1,
    question: "During a network packet capture audit, a security analyst discovers cleartext administrator credentials transmitted during remote router management sessions on TCP port 23. What secure protocol should replace Telnet to encrypt remote console access?",
    options: [
      { 
        text: "SSH (Secure Shell) on TCP Port 22", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Telnet is cleartext; SSH is encrypted! Telnet sends passwords in open plain text over port 23. SSH (Secure Shell) encrypts the entire command session on port 22."
  },
  {
    id: 2,
    question: "A website handles customer login forms over unencrypted HTTP on port 80. A security auditor warns that passwords can be stolen by anyone sniffing the Wi-Fi or network link. What secure protocol and port must be implemented?",
    options: [
      { 
        text: "HTTPS over TCP Port 443 with TLS", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "HTTPS encrypts web browsing! HTTP (port 80) sends data in the clear. HTTPS (port 443) uses TLS encryption so eavesdroppers cannot steal passwords or sensitive web data."
  },
  {
    id: 3,
    question: "An organization needs to securely upload daily financial files to an external cloud server. Traditional FTP exposes authentication credentials and data payloads in cleartext. What secure file transfer protocol uses SSH on port 22 to encrypt all transfers?",
    options: [
      { 
    ...
    laymanExplanation: "SFTP secures file transfers with SSH! Plain FTP exposes passwords and file contents. SFTP tunnels everything inside encrypted SSH sessions over port 22."
  },
  {
    id: 4,
    question: "An employee sets up an email client to download messages using unencrypted IMAP on TCP port 143. What secure version of IMAP should be selected to encrypt email client communications?",
    options: [
      { 
        text: "IMAPS (IMAP over TLS) on TCP Port 993", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "IMAPS encrypts email retrieval! Standard IMAP (port 143) sends emails in cleartext. IMAPS (port 993) uses TLS encryption to keep email downloads private."
  },
  {
    id: 5,
    question: "A traveling business executive connects to an open Wi-Fi network at an airport. To protect all laptop application communications—even those running insecure legacy protocols—from wireless sniffing, what transport security solution should be enabled?",
    options: [
      { 
        text: "Virtual Private Network (VPN) Encrypted Tunnel", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not a VPN tunnel." 
      },
      { 
        text: "Disabling WPA3 enterprise encryption", 
        isCorrect: false, 
        whyWrong: "Disabling WPA3 affects Wi-Fi security, not VPN tunnel creation." 
      },
      { 
        text: "Forcing CPU thermal throttling", 
        isCorrect: false, 
        whyWrong: "Thermal throttling is a hardware heat response, not a VPN tunnel." 
      }
    ],
    laymanExplanation: "VPNs create an encrypted pipe for all traffic! Even on untrusted public airport Wi-Fi, a VPN encrypts 100% of your internet connection back to corporate headquarters."
  },
  {
    id: 6,
    question: "A security engineer conducts a protocol audit to verify that no cleartext passwords are being transmitted over internal network switches. What tool is used to capture raw packets and inspect protocol headers?",
    options: [
      { 
        text: "Packet Capture Analyzer (e.g., Wireshark / tcpdump)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Packet capture analyzers reveal network truth! Tools like Wireshark capture raw network traffic so security teams can verify whether protocols are properly encrypted."
  }
];
...
export default function SecureProtocolsMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-blue-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            4.5 - Secure Protocols Practice 🔒
          </h1>
          <p class="text-blue-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering SSH (Port 22), HTTPS (Port 443), SFTP (Port 22), IMAPS (Port 993), VPN Tunnels, and Wireshark Packet Capture Audits!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-blue-50 border border-blue-200 rounded-2xl max-w-md mx-auto text-blue-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 4.5 Secure Protocols for CompTIA Security+!'
