...
const rawQuestions = [
  {
    id: 1,
    question: "An attacker attempts to log into thousands of different corporate employee accounts using only the single common password 'Winter2026!'. By trying only one guess per account every few hours, the attacker successfully compromises 8 accounts without triggering account lockout alerts. What attack is taking place?",
    options: [
      { 
        text: "Password Spraying Attack", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Password Spraying tests one popular password across thousands of accounts! By trying a common password like 'Winter2026!' just once per user, attackers avoid triggering '3 failed logins = account locked' rules, remaining completely undetected."
  },
  {
    id: 2,
    question: "An attacker steals a shadow password hash file from a Linux server and loads it onto a high-performance GPU cluster, computing billions of hash comparisons per second locally without connecting to the server. What type of attack is this?",
    options: [
      { 
        text: "Offline Brute Force Attack", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Online brute force attacks attempt logins over an active network connection and are slowed down by network latencies and lockout rules." 
      },
      { 
    ...
    laymanExplanation: "An Offline Brute Force attack cracks hashes locally on the attacker's computer! Because the attacker has a copy of the hash file, they can test billions of password combinations per second on fast graphics cards without being stopped by account lockout rules."
  },
  {
    id: 3,
    question: "A security audit reveals that a web portal saves user passwords in cleartext inside a SQL database. What is the primary danger of storing passwords in plaintext?",
    options: [
      { 
        text: "Anyone with access to the database or backup files instantly gains access to every user's cleartext password", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        text: "It causes high CPU thermal overheating on web servers", 
        isCorrect: false, 
        whyWrong: "Plaintext storage requires negligible CPU compute." 
      },
      { 
        text: "It automatically disables WPA3 enterprise wireless protection", 
        isCorrect: false, 
        whyWrong: "Database password storage does not affect WPA3 wireless settings." 
      },
      { 
        text: "It causes Layer 2 ethernet switching loops", 
        isCorrect: false, 
        whyWrong: "Switching loops occur on wired ethernet switches without STP." 
      }
    ],
    laymanExplanation: "Plaintext passwords mean zero privacy! If an attacker, rogue employee, or backup tape handler gets a copy of the database, every single password is completely readable without needing any cracking."
  },
  {
    id: 4,
    question: "What fundamental mathematical property of cryptographic hashing functions (such as SHA-256) makes them essential for secure password storage?",
    options: [
      { 
        text: "They are one-way functions that produce a fixed-length digest, making it mathematically impossible to reverse the digest back into the original plaintext password", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        text: "They automatically allow administrators to view forgotten passwords in cleartext", 
        isCorrect: false, 
        whyWrong: "Hashing prevents administrators from recovering cleartext passwords." 
      },
      { 
        text: "They compress video streams for faster web streaming", 
        isCorrect: false, 
        whyWrong: "Hashing is for cryptographic integrity and security, not video compression." 
      },
      { 
        text: "They broadcast Wi-Fi radio signals across long distances", 
        isCorrect: false, 
        whyWrong: "Hashing is a software math algorithm, not an RF antenna feature." 
      }
    ],
    laymanExplanation: "Hashes are a ONE-WAY trip! When you log in, the system hashes your input and compares it to the saved hash. Even if a hacker steals the stored hash, they cannot run the math in reverse to figure out your password."
  },
  {
    id: 5,
    question: "Which security controls directly protect a web login portal against automated Online Brute Force login attempts?",
    options: [
      { 
        text: "Account Lockout Policies, Rate Limiting, and CAPTCHA challenges", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
      ...
      { 
        ...
        isCorrect: false, 
        whyWrong: "Default passwords leave portals open to botnet takeovers." 
      }
    ],
    laymanExplanation: "Account lockout policies and CAPTCHAs stop online brute force! Lockouts block an IP/user after a few wrong tries, and CAPTCHA puzzles force automated bots to stop."
  },
  {
    id: 6,
    question: "What is the key operational difference between a traditional single-account brute-force attack and a password spraying attack?",
    options: [
      { 
        text: "Traditional brute force tries many passwords against one account (risking lockouts), while password spraying tries one password against many accounts (evading lockouts)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        text: "Traditional brute force requires physical access to server hardware", 
        isCorrect: false, 
        whyWrong: "Traditional brute force attacks are conducted over network login screens." 
      },
      { 
        text: "Password spraying requires administrative root privileges on the domain controller", 
        isCorrect: false, 
        whyWrong: "Password spraying requires no privileges; it tests public login portals." 
      },
      { 
        text: "Password spraying only targets unencrypted Telnet connections", 
        isCorrect: false, 
        whyWrong: "Password spraying targets web portals, SSH, RDP, and cloud logins." 
      }
    ],
    laymanExplanation: "Traditional brute force focuses deep on ONE target until the alarm goes off; Password spraying spreads out thin across THOUSANDS of targets so no single alarm goes off!"
  }
];
...
export default function PasswordAttacksMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            2.4 - Password Attacks Practice 🔑
          </h1>
          <p class="text-amber-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Password Spraying, Online vs Offline Brute Force, Plaintext Risks, One-Way Hashing, and Account Lockouts!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 2.4 Password Attacks for CompTIA Security+!'
