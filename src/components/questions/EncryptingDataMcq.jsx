...
const rawQuestions = [
  {
    id: 1,
    question: "A company mandates that all company-issued laptops must have their entire storage drives encrypted so that if a laptop is stolen, lost, or physically removed, the data at rest cannot be accessed by mounting the drive into another machine. What technology is deployed?",
    options: [
      { 
        text: "Full-Disk Encryption (BitLocker / FileVault)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        text: "SQL Injection", 
        isCorrect: false, 
        whyWrong: "SQL Injection is a web database attack technique, not a drive encryption mechanism." 
      },
      { 
        text: "Typosquatting", 
        isCorrect: false, 
    ...
    laymanExplanation: "Full-Disk Encryption (like Windows BitLocker or macOS FileVault) locks down the ENTIRE hard drive! If a thief steals the laptop or pulls out the hard drive, they can't read a single file without the decryption key."
  },
  {
    id: 2,
    question: "A database administrator needs to protect credit card numbers stored in a shared customer database. Rather than encrypting the entire database file, the administrator encrypts only the specific credit card column using unique symmetric keys. What database encryption method is used?",
    options: [
      { 
        text: "Record-Level / Column-Level Database Encryption", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Column-Level Encryption lets you pick and choose! Instead of encrypting everything, you encrypt only the sensitive columns (like SSNs or Credit Cards) while leaving normal columns unencrypted for faster searching."
  },
  {
    id: 3,
    question: "An organization needs to securely connect its main headquarters network to a remote branch office network across the public Internet, encrypting all network traffic traversing the link regardless of application. What transport encryption solution is implemented?",
    options: [
      { 
        text: "Site-to-Site IPsec VPN", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "A Site-to-Site IPsec VPN creates an encrypted tunnel between two whole offices over the Internet! All computers in Office A can talk safely to computers in Office B as if they were on the same local network."
  },
  {
    id: 4,
    question: "A developer implements user password storage. To prevent attackers from using high-speed GPU cracking tools to guess weak passwords offline, the developer hashes each password multiple thousands of times using PBKDF2. What cryptographic technique is applied?",
    options: [
      { 
        text: "Key Stretching (Key Strengthening)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Key Stretching makes weak passwords strong! By running a password through 100,000 hashing rounds (like PBKDF2 or bcrypt), it forces hackers to spend huge amounts of time for every single password guess."
  },
  {
    id: 5,
    question: "A security analyst compares a 256-bit AES symmetric key against a 3,072-bit RSA asymmetric key. Why does asymmetric encryption require significantly larger key lengths to achieve comparable security to symmetric encryption?",
    options: [
      { 
        text: "Asymmetric algorithms rely on mathematical prime structures vulnerable to specialized mathematical shortcut algorithms", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not a key size mathematical property." 
      },
      { 
    ...
    laymanExplanation: "Asymmetric keys need to be HUGE! Because RSA asymmetric encryption uses prime numbers, mathematicians have found shortcuts to factor them, requiring RSA keys to be 3,072+ bits compared to 256 bits for AES."
  },
  {
    id: 6,
    question: "According to Kerckhoffs's principle, modern cryptographic security relies on public, open-source algorithms (such as AES or RSA) where all mathematical details are known to the public. What single component must remain secret to guarantee confidentiality?",
    options: [
      { 
        text: "The Cryptographic Secret Key", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Kerckhoffs's Principle says: 'Never rely on secret code!' Security shouldn't come from hiding the math formula — true security comes strictly from keeping your cryptographic KEY secret."
  }
];
...
export default function EncryptingDataMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-amber-600 via-yellow-600 to-amber-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            1.4 - Encrypting Data Practice 🔐
          </h1>
          <p class="text-amber-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Data at Rest (BitLocker/FileVault), Database Column Encryption, IPsec VPNs, Key Lengths (Symmetric vs Asymmetric), Key Stretching (PBKDF2), and Kerckhoffs's Principle!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 1.4 Encrypting Data for CompTIA Security+!'
