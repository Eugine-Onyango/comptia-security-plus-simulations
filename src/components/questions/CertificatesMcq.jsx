...
const rawQuestions = [
  {
    id: 1,
    question: "A web administrator generates a key pair on the company web server, then sends the public key along with identifying information about the company to a trusted third-party organization for validation and digital signing. What process is being performed?",
    options: [
      { 
      ...
      { 
        text: "SQL Injection", 
        isCorrect: false, 
        whyWrong: "SQL Injection is a web database attack technique, not a certificate enrollment process." 
      },
      { 
        text: "Typosquatting", 
        isCorrect: false, 
    ...
    laymanExplanation: "A CSR is like a job application for trust! You create your key pair, bundle your public key with your company info, and send it to a Certificate Authority (CA) to review and digitally sign — proving to the world that your website is legit."
  },
  {
    id: 2,
    question: "A browser connects to an HTTPS banking website and needs to verify the website's digital certificate hasn't been revoked due to a compromise. Instead of downloading a massive revocation list file, the browser sends a quick, lightweight HTTP query to check the certificate's real-time status. What protocol is used?",
    options: [
      { 
    ...
    laymanExplanation: "OCSP is a quick certificate health check! Instead of downloading a huge Certificate Revocation List (CRL) file, the browser sends a fast HTTP ping asking 'Is this certificate still good?' and gets an instant yes-or-no reply."
  },
  {
    id: 3,
    question: "A medium-sized enterprise builds its own internal Certificate Authority using Windows Certificate Services. The IT team installs the CA root certificate onto all corporate laptops, servers, and mobile devices. What type of certificate infrastructure is deployed?",
    options: [
      { 
        text: "Private Certificate Authority (Internal / Self-Signed CA)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "A Private CA means YOU become your own certificate authority! Companies build in-house CAs for internal servers and devices. Since your devices already trust your internal CA root certificate, you don't need to pay for public certificates."
  },
  {
    id: 4,
    question: "An organization needs a single SSL/TLS certificate to secure all subdomains under its domain (e.g., mail.example.com, shop.example.com, api.example.com). Instead of buying separate certificates for each subdomain, they purchase one certificate using '*.example.com' notation. What certificate type is used?",
    options: [
      { 
    ...
    laymanExplanation: "A Wildcard Certificate is one certificate that covers ALL subdomains! The asterisk (*) in '*.example.com' means it secures mail.example.com, shop.example.com, api.example.com — all with one purchase!"
  },
  {
    id: 5,
    question: "After the Heartbleed vulnerability (CVE-2014-0160) exposed private keys on millions of web servers in April 2014, Certificate Authorities mass-revoked compromised certificates and published updated lists of invalidated certificates. What revocation mechanism was used?",
    options: [
      { 
      ...
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not a certificate revocation method." 
      },
      { 
    ...
    laymanExplanation: "A Certificate Revocation List (CRL) is a 'blacklist' of dead certificates! After Heartbleed, every compromised certificate was added to the CRL so browsers would reject them. CRLs can get huge and are maintained by the CA."
  },
  {
    id: 6,
    question: "A web server includes its OCSP revocation status response — pre-signed by the Certificate Authority — directly inside the TLS handshake when a browser connects. This eliminates the need for the browser to contact the CA's OCSP responder separately. What technique is described?",
    options: [
      { 
        text: "OCSP Stapling", 
        isCorrect: true, 
    ...
    laymanExplanation: "OCSP Stapling is like bringing your own proof of good standing! The web server pre-fetches its certificate status from the CA and 'staples' it into the TLS handshake, so the browser doesn't need to make a separate trip to the CA."
  }
];
...
export default function CertificatesMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-amber-600 via-yellow-600 to-amber-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            1.4 - Certificates Practice 📜✅
          </h1>
          <p class="text-amber-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering CSRs, Certificate Authorities (CA), Wildcard Certificates, CRL, OCSP, OCSP Stapling, and Private CAs!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 1.4 Certificates for CompTIA Security+!'
