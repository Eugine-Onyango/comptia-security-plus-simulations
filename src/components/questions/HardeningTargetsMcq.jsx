...
const rawQuestions = [
  {
    id: 1,
    question: "An electrical utility operates a SCADA/ICS system that monitors power grid distribution across several cities. To protect the industrial control equipment from external cyberattacks, what network hardening strategy must be implemented?",
    options: [
      { 
        text: "Extensive Network Segmentation and Air-gapping (No Direct Internet Access)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "SCADA and Industrial Control Systems (ICS) run heavy machinery like power plants and water facilities! Because these systems were not designed for internet threats, you must strictly segment and air-gap them so hackers on the outside can never reach them."
  },
  {
    id: 2,
    question: "A defense manufacturer uses a Real-Time Operating System (RTOS) to control automated missile guidance testing equipment requiring deterministic processing schedules. How should the security team harden the RTOS endpoints without causing processing delays?",
    options: [
      { 
        text: "Isolate the system on a dedicated network segment, run minimal required services, and apply host-based firewalls", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "An RTOS requires microsecond timing! You cannot run heavy background scans that cause delays. Hardening an RTOS means stripping unnecessary services down to the bare minimum and isolating it behind strict network firewalls."
  },
  {
    id: 3,
    question: "An organization deploys 50 smart security cameras and smart thermostats throughout its headquarters. What two hardening practices prevent an attacker who infects a camera from pivoting to the corporate financial database?",
    options: [
      { 
        text: "Change factory default passwords immediately and isolate IoT devices on their own VLAN", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "IoT devices (like smart cameras) often have weak default passwords and rarely get security updates! Changing the default password and locking them on a separate IoT VLAN ensures a hacked camera cannot reach your important company files."
  },
  {
    id: 4,
    question: "A Cloud Administrator uses a dedicated laptop to manage an enterprise AWS environment via Web Consoles and API CLI keys. Why is hardening the administrator's workstation with EDR, MFA, and full disk encryption considered critical?",
    options: [
      { 
        text: "The cloud management workstation holds the 'keys to the kingdom' for the entire cloud infrastructure", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "The admin laptop has total control! If a hacker compromises the cloud admin workstation, they steal API tokens that grant complete access to all cloud servers and databases."
  },
  {
    id: 5,
    question: "A healthcare provider allows doctors to access electronic health records on personal tablets. What solution allows IT to enforce security patches, mandate PIN locks, and separate corporate health data from personal apps?",
    options: [
      { 
        text: "Mobile Device Management (MDM) with Containerization", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "MDM manages mobile phones and tablets! Containerization keeps business medical records inside a secure, encrypted bubble on the phone—completely separate from personal photos and social media apps."
  },
  {
    id: 6,
    question: "A network engineer is hardening enterprise core switches and routers. In addition to changing default passwords, which configuration change prevents attackers from sniffing administrative credentials across the network?",
    options: [
      { 
        text: "Disabling unencrypted HTTP and Telnet management in favor of HTTPS and SSH", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Never use cleartext management protocols! Telnet (port 23) and HTTP (port 80) send passwords over the wire in plain text. Hardening network gear requires using SSH (port 22) and HTTPS (port 443) so credentials are fully encrypted."
  }
];
...
export default function HardeningTargetsMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-blue-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            4.1 - Hardening Targets Practice 🎯
          </h1>
          <p class="text-blue-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Mobile MDM, Workstations, Network Gear, Cloud Management, SCADA/ICS Air-gaps, RTOS, and IoT VLAN Isolation!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-blue-50 border border-blue-200 rounded-2xl max-w-md mx-auto text-blue-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 4.1 Hardening Targets for CompTIA Security+!'
