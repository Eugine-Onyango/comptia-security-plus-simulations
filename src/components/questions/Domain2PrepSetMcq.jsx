...
import { DOMAIN2_PREPSET_QUESTIONS } from '../../data/domain2PrepSetQuestions';
...
export default function Domain2PrepSetMcq({ onBack }) {
  ...
  const initQuiz = () => {
    const prepared = DOMAIN2_PREPSET_QUESTIONS.map(q => ({
      ...q,
  ...
  return (
    <div className="max-w-4xl mx-auto px-3 sm:px-4 py-6 space-y-6 animate-fadeIn">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <span className="px-3 py-1 bg-rose-100 text-rose-950 border border-rose-300 rounded-full font-black text-xs flex items-center gap-1.5">
            ...
            <span>PrepSet Exam Simulator • Domain 2.0 (Threats & Mitigations)</span>
          </span>
        </div>
      </div>
      ...
      {!showResult ? (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-3xl p-5 sm:p-6 border-4 border-rose-300 shadow-md space-y-4">
            ...
            <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden border border-slate-200">
              <div 
                className="bg-gradient-to-r from-rose-400 to-red-600 h-full transition-all duration-300 rounded-full"
          ...
          <div className="space-y-3">
            {currentQ.options.map((option, idx) => {
              ...
              if (isSelected && !isSubmitted) {
                btnStyle = "bg-rose-50 border-rose-500 text-rose-950 ring-2 ring-rose-400/40 shadow-md";
              }
          ...
          <div className="flex items-center justify-between gap-3 pt-2">
            {!isSubmitted ? (
              ...
              <button
                onClick={handleNext}
                className="w-full sm:w-auto px-8 py-3.5 rounded-2xl bg-rose-600 hover:bg-rose-700 text-white font-black text-xs sm:text-sm transition-all shadow-md active:scale-95 cursor-pointer flex items-center justify-center gap-2"
              >
        ...
        /* Results & Score Card */
        <div className="bg-white rounded-3xl p-6 sm:p-8 border-4 border-rose-300 shadow-lg space-y-6 text-center animate-fadeIn">
          
          <div className="w-20 h-20 mx-auto rounded-3xl bg-rose-500 text-white flex items-center justify-center shadow-lg transform -rotate-3">
          ...
          <div className="flex items-center justify-center gap-3 pt-4">
            <button
              ...
              className="px-6 py-3 rounded-2xl bg-rose-600 hover:bg-rose-700 text-white font-black text-xs sm:text-sm transition-all active:scale-95 flex items-center gap-2 cursor-pointer shadow-md"
            >
              <RotateCcw className="w-4 h-4" />
              <span>Retry Domain 2.0</span>
            </button>
