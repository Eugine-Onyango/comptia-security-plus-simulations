...
const rawQuestions = [
  {
    id: 1,
    question: "Bob wants to send a confidential, encrypted document to Alice across the Internet. According to asymmetric public key cryptography, which key must Bob use to encrypt the document so that only Alice can read it?",
    options: [
      { 
      ...
      { 
        text: "SQL Injection", 
        isCorrect: false, 
        whyWrong: "SQL Injection is a web database attack technique, not an encryption key." 
      },
      { 
        text: "Typosquatting", 
        isCorrect: false, 
    ...
    laymanExplanation: "To send a secret message to Alice, encrypt it with ALICE'S PUBLIC KEY! Anyone can see Alice's public key, but only Alice holds the matching private key required to unlock and read the message."
  },
  {
    id: 2,
    question: "Alice receives an encrypted document that Bob locked using Alice's public key. Which key must Alice use to decrypt the ciphertext back into readable plaintext?",
    options: [
      { 
    ...
    laymanExplanation: "Only Alice's PRIVATE KEY can decrypt data that was encrypted with Alice's public key! Alice keeps her private key strictly secret on her machine."
  },
  {
    id: 3,
    question: "An organization establishes a framework of policies, procedures, software, and hardware to bind public keys to employee identities, issue digital certificates, and manage key revocations. What system architecture is established?",
    options: [
      { 
        text: "Public Key Infrastructure (PKI) managed by a Certificate Authority (CA)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "PKI (Public Key Infrastructure) is the entire ecosystem that manages digital certificates! A trusted Certificate Authority (CA) verifies identities and signs public key certificates."
  },
  {
    id: 4,
    question: "An enterprise requires copies of all employee private decryption keys to be securely stored with an authorized third-party custodian so company files can be recovered under court order or if an employee departs unexpectedly. What arrangement is implemented?",
    options: [
      { 
        text: "Key Escrow", 
        isCorrect: true, 
    ...
    laymanExplanation: "Key Escrow holds a backup copy of private decryption keys in a safe vault! If an employee leaves or loses their key, key escrow allows authorized managers to recover encrypted business data."
  },
  {
    id: 5,
    question: "Symmetric encryption algorithms (like AES) use a single shared key and are extremely fast for encrypting large files, while asymmetric encryption is slower but handles key exchange securely. How do modern protocols like HTTPS/TLS combine both?",
    options: [
      { 
        text: "Asymmetric encryption securely exchanges a temporary symmetric session key, then fast symmetric encryption encrypts the bulk data flow", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not a cryptographic hybrid protocol." 
      },
      { 
    ...
    laymanExplanation: "Best of both worlds! Hybrid cryptography uses slow asymmetric encryption just to exchange a secret session key, then switches to super-fast symmetric encryption for the rest of the web connection."
  },
  {
    id: 6,
    question: "An attacker intercepts a web server's public key from an SSL/TLS digital certificate. Why is it computationally infeasible for the attacker to derive or calculate the matching private key from the public key?",
    options: [
      { 
        text: "Asymmetric key generation relies on complex one-way mathematical functions using extremely large prime numbers", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Public key math is a one-way street! Because key pairs are generated using massive prime numbers and complex trapdoor math, you can freely share the public key without anyone being able to figure out your private key."
  }
];
...
export default function PublicKeyInfrastructureMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-amber-600 via-yellow-600 to-amber-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            1.4 - Public Key Infrastructure Practice 🔑
          </h1>
          <p class="text-amber-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Asymmetric vs Symmetric Encryption, Public & Private Keys, Certificate Authorities (CA), Key Escrow, and Hybrid Encryption!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 1.4 Public Key Infrastructure for CompTIA Security+!'
