...
const rawQuestions = [
  {
    id: 1,
    question: "An attacker captures a wireless four-way handshake on a corporate WPA2-PSK network and uses GPU password cracking software to recover the Wi-Fi password offline. What WPA3 feature replaces PSK with a Diffie-Hellman derived key exchange (Dragonfly Handshake) to prevent offline dictionary attacks?",
    options: [
      { 
    ...
    laymanExplanation: "SAE (Simultaneous Authentication of Equals) fixes the WPA2 password vulnerability! Instead of sending hashes that attackers can crack offline with fast GPUs, WPA3 SAE uses the Dragonfly Diffie-Hellman handshake so password guessing attacks fail instantly."
  },
  {
    id: 2,
    question: "WPA3 utilizes Galois/Counter Mode Protocol (GCMP) for strong AES encryption. What component of GCMP provides Message Integrity Checks (MIC) to ensure received wireless packets have not been modified in transit?",
    options: [
      { 
        text: "Galois Message Authentication Code (GMAC)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "GMAC acts like a digital tamper-evident seal! Along with AES encryption for confidentiality, GMAC calculates a message authentication code so the receiver knows the wireless data wasn't changed on-path."
  },
  {
    id: 3,
    question: "A company requires all employees to authenticate to the corporate Wi-Fi network using their individual Active Directory usernames and passwords rather than a shared company password. What wireless security mode must be configured on the access points?",
    options: [
      { 
        text: "WPA3-Enterprise (WPA3-802.1X with a RADIUS Server)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Use WPA3-Enterprise for companies! WPA3-Personal uses one shared password for everyone, while WPA3-Enterprise connects to a backend RADIUS server so each user logs in with their own individual credentials."
  },
  {
    id: 4,
    question: "In an IEEE 802.1X wireless architecture, a user's laptop connects to a Wireless Access Point (WAP), which proxies the authentication request to a backend RADIUS server. In 802.1X terminology, what role is played by the Wireless Access Point?",
    options: [
      { 
    ...
    laymanExplanation: "Know your 802.1X roles! The client laptop is the Supplicant, the backend RADIUS server is the Authentication Server, and the Wireless Access Point in the middle gating access is the Authenticator."
  },
  {
    id: 5,
    question: "A network administrator checks central RADIUS logs to audit how long a contractor remained connected to the network, their exact login/logout timestamps, and total bytes transferred. Which arm of the AAA framework is being inspected?",
    options: [
      { 
        text: "Accounting", 
        isCorrect: true, 
      ...
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency does not record user login time or byte counts." 
      },
      { 
        text: "Disabling WPA3 enterprise encryption", 
        isCorrect: false, 
        whyWrong: "Disabling WPA3 affects wireless security, not AAA accounting logs." 
      },
      { 
        text: "Forcing CPU thermal throttling", 
        isCorrect: false, 
        whyWrong: "Thermal throttling is a hardware heat response, not an AAA pillar." 
      }
    ],
    laymanExplanation: "Accounting tracks who did what and for how long! In AAA (Authentication, Authorization, Accounting), Accounting keeps detailed audit logs of login times, session durations, and data usage."
  },
  {
    id: 6,
    question: "On legacy WPA2-PSK networks, an attacker who learns the shared Wi-Fi password can capture and decrypt traffic sent by all other connected users. How does WPA3 SAE solve this lack of forward secrecy?",
    options: [
      { 
        text: "SAE generates unique session keys for every device, preventing decryption of other users' traffic", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "WPA3 SAE gives everyone their own unique encryption key! Even if everyone knows the Wi-Fi password, SAE scrambles each user's data with a unique session key so users cannot spy on each other."
  }
];
...
export default function WirelessSecuritySettingsMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-blue-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            4.1 - Wireless Security Settings Practice 🔐
          </h1>
          <p class="text-blue-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering WPA3 & GCMP/GMAC, SAE Dragonfly Handshakes, WPA3-Enterprise RADIUS, 802.1X Roles, and AAA Accounting!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-blue-50 border border-blue-200 rounded-2xl max-w-md mx-auto text-blue-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 4.1 Wireless Security Settings for CompTIA Security+!'
