...
const rawQuestions = [
  {
    id: 1,
    question: "A retail company handling credit card payments must separate its Point of Sale (POS) network from the general employee Wi-Fi network to comply with PCI DSS standards and limit audit scope. What network architecture technique should the company implement?",
    options: [
      { 
        text: "Network Segmentation via VLANs and Firewalls", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Network Segmentation isolates sensitive networks! By placing payment processing systems on their own virtual network (VLAN) behind a firewall, hackers on the general employee Wi-Fi cannot jump over to steal credit card data."
  },
  {
    id: 2,
    question: "A system administrator wants to prevent user workstations from directly accessing backend SQL database servers. They configure firewall rules allowing only web app servers to talk to the database on TCP port 1433. What design principle is being applied?",
    options: [
      { 
        text: "Tiered Network Segmentation / Least Privilege Network Access", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Tiered segmentation keeps databases safe! Users talk to the web server, and ONLY the web server is allowed to talk to the database. If a user's laptop gets infected, the malware cannot directly probe or hack the database."
  },
  {
    id: 3,
    question: "An engineer creates a new Network Access Control List (ACL) on a router to manage SSH remote management access. After applying the rule `deny ip any any` at the bottom of the list, the engineer's active SSH terminal session disconnects and further logins fail. What caused this?",
    options: [
      { 
        text: "Accidental lockout due to missing an explicit permit rule for the administrator's IP prior to the implicit deny", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        text: "The router suffered a physical power supply failure", 
        isCorrect: false, 
        whyWrong: "The SSH disconnect occurred instantly due to the software ACL policy application." 
      },
      { 
    ...
    laymanExplanation: "Be super careful with ACLs! Routers evaluate rules from top to bottom. If you add a 'deny all' rule without putting 'allow admin IP' above it first, the router immediately blocks YOU and locks you out of remote management."
  },
  {
    id: 4,
    question: "An administrator wants to prevent rogue or unapproved software from running on corporate laptops. They configure a security policy that calculates the unique SHA-256 hash of approved binaries and blocks execution if the hash doesn't match—even if an attacker renames `malware.exe` to `chrome.exe`. What control is this?",
    options: [
      { 
        text: "Application Allow Listing by Application Hash", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Application Hashing acts like a digital fingerprint checker! Even if a hacker renames a malicious file to look like a safe app, the system checks the file's fingerprint (hash). If it's not on the approved list, it refuses to run!"
  },
  {
    id: 5,
    question: "A company configures Windows Defender Application Control (WDAC) to automatically permit any application binary that carries a valid cryptographic digital signature from verified publishers like Microsoft, Adobe, or Cisco. What rule type is being used?",
    options: [
      { 
        text: "Certificate / Publisher Signature Allow List Rule", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Publisher Certificate rules check digital ID cards! Instead of hashing every single updated file version, the operating system trusts any application signed by legitimate, verified software companies like Microsoft or Cisco."
  },
  {
    id: 6,
    question: "A firewall rule evaluates incoming packets based on Source IP (`192.168.1.0/24`), Destination IP (`10.0.0.5`), Destination Port (`443`), and Protocol (`TCP`). What type of security filter is executing these inspection rules?",
    options: [
      { 
    ...
    laymanExplanation: "Network Access Control Lists (ACLs) are network bouncers! They inspect every packet's Source IP, Destination IP, and Port Number against a list of rules to decide whether to let the traffic in or block it."
  }
];
...
export default function SegmentationAccessControlMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            2.5 - Segmentation and Access Control Practice 🛡️
          </h1>
          <p class="text-amber-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Network Segmentation (PCI DSS), Tiered Architecture, Network ACLs & Lockouts, Application Hash Allow Lists, Certificate Publisher Rules, and OS Control Lists!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 2.5 Segmentation and Access Control for CompTIA Security+!'
