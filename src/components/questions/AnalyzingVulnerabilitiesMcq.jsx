...
const rawQuestions = [
  {
    id: 1,
    question: "A vulnerability scanner alerts an administrator that a production server contains a critical IIS web server vulnerability. Upon manual inspection, the administrator confirms that IIS is completely uninstalled and the server runs Nginx instead. What type of alert occurred?",
    options: [
      { 
        text: "False Positive", 
        isCorrect: true, 
    ...
    laymanExplanation: "False Positives are false alarms! The scanner incorrectly claims a bug exists when it really doesn't. Security teams must verify scanner findings before acting."
  },
  {
    id: 2,
    question: "A security analyst reviews a vulnerability report on the NIST National Vulnerability Database. A flaw is assigned a quantitative severity rating of 9.8 out of 10.0. What standardized rating system is being used?",
    options: [
      { 
    ...
    laymanExplanation: "CVSS ranks bug danger on a scale from 0 to 10! The Common Vulnerability Scoring System gives every flaw a standardized number so security teams know which bugs are most critical."
  },
  {
    id: 3,
    question: "The exact same unpatched software flaw exists on Server A (a public cloud production database handling live customer payments) and Server B (an isolated internal test lab server). Why does Server A receive immediate patching priority?",
    options: [
      { 
        text: "Environmental variables and threat exposure differ significantly between internet-facing production and isolated test lab systems", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Location matters! A bug on a public internet server processing money is far more dangerous than the same bug locked away on an isolated test computer."
  },
  {
    id: 4,
    question: "A risk assessment report indicates that a successful buffer overflow attack on a core database would destroy 100% of the asset's business value. What metric is expressed by this percentage?",
    options: [
      { 
    ...
    laymanExplanation: "Exposure Factor (EF) measures damage as a percentage! An EF of 100% means an attack completely destroys or wipes out the affected asset."
  },
  {
    id: 5,
    question: "A security analyst searches MITRE and NIST databases for vulnerability information using a standardized identifier string formatted as 'CVE-2023-38606'. What is the purpose of this identifier?",
    options: [
      { 
        text: "To uniquely index and cross-reference known software vulnerabilities across global security databases", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not a bug identifier string." 
      },
      { 
        text: "Disabling WPA3 enterprise encryption", 
        isCorrect: false, 
        whyWrong: "Disabling WPA3 affects Wi-Fi security, not vulnerability indexing." 
      },
      { 
        text: "Forcing CPU thermal throttling", 
        isCorrect: false, 
        whyWrong: "Thermal throttling is a hardware heat response, not a vulnerability index." 
      }
    ],
    laymanExplanation: "CVE IDs give every vulnerability a unique name tag! Common Vulnerabilities and Exposures (CVE) numbers let security tools and databases talk about the exact same bug."
  },
  {
    id: 6,
    question: "An organization's patch policy mandates testing all vendor security updates in a staging environment before production deployment, accepting short-term vulnerability exposure to avoid system crashes. What concept is being managed?",
    options: [
      { 
        text: "Risk Tolerance vs Patch Deployment Staging", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Risk tolerance balances speed vs stability! Companies test patches before installing them on live servers because a rushed patch could crash business applications."
  }
];
...
export default function AnalyzingVulnerabilitiesMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-blue-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            4.3 - Analyzing Vulnerabilities Practice 📊
          </h1>
          <p class="text-blue-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering False Positives vs Negatives, CVSS Ratings, Environmental Variables, Exposure Factor (EF), CVE Identifiers, and Risk Tolerance!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-blue-50 border border-blue-200 rounded-2xl max-w-md mx-auto text-blue-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 4.3 Analyzing Vulnerabilities for CompTIA Security+!'
