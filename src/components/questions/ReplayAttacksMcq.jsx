...
const rawQuestions = [
  {
    id: 1,
    question: "An attacker extracts an NTLM password hash from a compromised Windows workstation's RAM memory and transmits that exact hash directly to a domain controller to authenticate as Domain Admin—without ever cracking or knowing the user's actual plaintext password. What attack is taking place?",
    options: [
      { 
        text: "Pass the Hash (PtH) Attack", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
      ...
      { 
        text: "Typosquatting", 
        isCorrect: false, 
        whyWrong: "Typosquatting involves registering misspelled web domain URLs." 
      },
      { 
        text: "RF Jamming", 
        isCorrect: false, 
    ...
    laymanExplanation: "Pass the Hash (PtH) lets hackers bypass password guessing! Because Windows systems authenticate using password hashes, an attacker who steals your saved hash can replay that hash directly to log in as you without ever needing your actual password."
  },
  {
    id: 2,
    question: "While sitting in a coffee shop using unencrypted public Wi-Fi, a user logs into a web forum. An attacker running a packet sniffer captures the cleartext HTTP session cookie containing Session ID '3B0027A38FDF37' and injects it into their own browser to gain instant access to the account. What attack occurred?",
    options: [
      { 
        text: "Session Hijacking (Sidejacking)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Session Hijacking (Sidejacking) steals your active session key! When you log in over unencrypted HTTP, your browser sends a Session ID cookie. A hacker nearby can sniff that cookie off the airwaves and load it into their own browser to act as you!"
  },
  {
    id: 3,
    question: "Which cryptographic mechanism prevents replay attacks by ensuring that every authentication request includes a unique single-use number and strict time window so re-transmitted data is immediately rejected by the server?",
    options: [
      { 
        text: "Nonces and Time-stamps", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Nonces (Number used ONCE) and Timestamps stop replay attacks! It's like a ticket stamped with 'valid for 30 seconds only'. If a hacker steals and replays the packet later, the server says 'This ticket was already used and is expired!' and drops it."
  },
  {
    id: 4,
    question: "Why is a classic Replay Attack distinguished from an On-path (Man-in-the-Middle) attack once the initial transmission packet capture phase is finished?",
    options: [
      { 
        text: "The actual replaying of captured credentials/tokens can be performed later from a completely separate machine without remaining on-path", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        text: "Replay attacks require 100% physical access to server room HVAC units", 
        isCorrect: false, 
        whyWrong: "HVAC units handle environmental server room cooling." 
      },
      { 
        text: "Replay attacks only operate on unshielded microwave ovens", 
        isCorrect: false, 
        whyWrong: "Microwave ovens cause unintentional RF interference." 
      },
      { 
        text: "Replay attacks cannot be logged by network firewalls", 
        isCorrect: false, 
        whyWrong: "Firewalls and SIEMs log incoming network connection attempts." 
      }
    ],
    laymanExplanation: "A Replay Attack does NOT require staying in the middle! Once a hacker sniffs your valid log-in data or token, they can disconnect, go home, and re-send that stolen data days later from their own computer."
  },
  {
    id: 5,
    question: "An ethical hacker uses tools like Tamper Data or Cookies Manager+ to intercept an HTTP POST request in transit, changing the user's role header from 'Role=User' to 'Role=Admin' before sending it to the server. What technique is this?",
    options: [
      { 
        text: "Header and Cookie Manipulation", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Physical brute force involves picking mechanical door locks." 
      },
      { 
    ...
    laymanExplanation: "Header & Cookie Manipulation alters HTTP request data on the fly! Using browser extension tools, an attacker can modify fields like 'Role' or 'User_ID' before the request hits the server to trick the app into giving them higher privileges."
  },
  {
    id: 6,
    question: "What is the primary countermeasure to prevent attackers on local networks from sniffing session cookies off the wire and executing sidejacking attacks?",
    options: [
      { 
        text: "Enforcing HTTPS / End-to-End TLS Encryption", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        text: "Allowing HTTP cleartext fallback connections", 
        isCorrect: false, 
        whyWrong: "HTTP cleartext connections allow anyone to sniff plain text session cookies." 
      },
      { 
        text: "Disabling Spanning Tree Protocol (STP)", 
        isCorrect: false, 
        whyWrong: "Disabling STP creates Layer 2 network switching loops." 
      },
      { 
        text: "Using default manufacturer passwords on routers", 
        isCorrect: false, 
        whyWrong: "Default credentials expose hardware to automated botnet logins." 
      }
    ],
    laymanExplanation: "HTTPS / End-to-End TLS Encryption locks your session cookies inside an encrypted tunnel! When websites enforce HTTPS-only connections, hackers listening on the public Wi-Fi see only unreadable scrambled gibberish."
  }
];
...
export default function ReplayAttacksMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            2.4 - Replay Attacks Practice 🔄
          </h1>
          <p class="text-amber-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Replay Attacks, Pass the Hash (PtH), Session Hijacking / Sidejacking, Nonces & Timestamps, Header/Cookie Manipulation, and HTTPS Protections!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 2.4 Replay Attacks for CompTIA Security+!'
