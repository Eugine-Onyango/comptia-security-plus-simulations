...
const rawQuestions = [
  {
    id: 1,
    question: "A high-security data center installs an entryway featuring two interlocking doors where opening the outer door automatically locks the inner door until identity verification completes. What physical security control prevents tailgating by allowing only one person through at a time?",
    options: [
      { 
      ...
      { 
        text: "SQL Injection", 
        isCorrect: false, 
        whyWrong: "SQL Injection is a web database attack technique, not a physical security entryway." 
      },
      { 
        text: "Typosquatting", 
        isCorrect: false, 
    ...
    laymanExplanation: "An Access Control Vestibule (Mantrap) is a double-door airlock security system! When Door 1 opens, Door 2 stays locked. This stops unbadged tailgaters from slipping inside behind employees."
  },
  {
    id: 2,
    question: "A bank headquarters installs heavy concrete posts and steel pillars along the sidewalk in front of its glass lobby. The pillars allow pedestrians to walk freely while physically stopping hostile vehicle ramming attacks. What physical control is installed?",
    options: [
      { 
        text: "Bollards and Barricades", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Bollards are heavy concrete or steel posts anchored into the ground! They let people walk through easily while physically blocking cars and trucks from smashing into buildings."
  },
  {
    id: 3,
    question: "A government data vault requires two separate authorized security managers to scan their access badges simultaneously before the main vault lock releases. No single manager can open the vault alone. What security principle minimizes insider exposure?",
    options: [
      { 
        text: "Two-Person Integrity / Control", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Two-Person Integrity means 'It takes two to tango!' Requiring two people to approve an action prevents any single employee from stealing high-value assets or abusing power."
  },
  {
    id: 4,
    question: "An intruder steps onto a sensitive data room floor mat, triggering an immediate security alarm due to the physical force applied to the floor surface. Which type of physical sensor detected the intruder?",
    options: [
      { 
    ...
    laymanExplanation: "Pressure sensors detect weight and force changes! Underneath floor mats or window glass, pressure sensors trigger alarms when someone steps on them or breaks a window."
  },
  {
    id: 5,
    question: "A facility security director installs high-lumen perimeter floodlights angled to eliminate dark shadows along the building exterior, assisting CCTV cameras with clear facial recognition. What sensor type emits high-frequency sound waves and measures reflected signals to detect motion inside dark indoor hallways?",
    options: [
      { 
        text: "Ultrasonic Motion Sensor", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network speed metric, not a physical sensor." 
      },
      { 
    ...
    laymanExplanation: "Ultrasonic sensors work like bat echolocation! They bounce high-frequency sound waves off walls and detect motion when something disrupts the returning echo."
  },
  {
    id: 6,
    question: "A logistics facility updates its CCTV security camera network with automated software that reads license plates at vehicle gates and alerts guards to unrecognized faces in restricted areas. What camera feature replaces or enhances human guard monitoring?",
    options: [
      { 
        text: "Video Analytics (License Plate & Facial Object Recognition)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Modern CCTV cameras have smart AI analytics! They automatically read car license plates, detect human faces, and trigger security alerts without requiring guards to stare at screens 24/7."
  }
];
...
export default function PhysicalSecurityMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-amber-600 via-yellow-600 to-amber-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            1.2 - Physical Security Practice 🏢
          </h1>
          <p class="text-amber-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Bollards, Access Control Vestibules (Mantraps), Two-Person Integrity, Video Surveillance (CCTV Analytics), Perimeter Lighting, and Motion Sensors!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 1.2 Physical Security for CompTIA Security+!'
