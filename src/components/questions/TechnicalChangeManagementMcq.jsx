...
const rawQuestions = [
  {
    id: 1,
    question: "A security administrator configures an endpoint protection policy that blocks all unapproved software applications from executing by default. Only binary executables explicitly signed and listed on a master approval list are permitted to run. What technical control is implemented?",
    options: [
      { 
        text: "Application Allow List (Whitelisting)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        text: "SQL Injection", 
        isCorrect: false, 
        whyWrong: "SQL Injection is a web database attack technique, not an application execution control." 
      },
      { 
        text: "Typosquatting", 
        isCorrect: false, 
    ...
    laymanExplanation: "An Application Allow List (Whitelisting) is ultra-strict! It blocks EVERYTHING by default. Nothing can execute unless it is explicitly placed on the pre-approved good list."
  },
  {
    id: 2,
    question: "A network engineer needs to upgrade a critical enterprise firewall cluster without interrupting user web traffic. The engineer switches active traffic to the secondary standby firewall, upgrades the primary firewall, tests it, and then switches traffic back. What technical downtime strategy is used?",
    options: [
      { 
        text: "High Availability (HA) Failover Redundancy", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "HA Failover prevents downtime! By shifting live traffic to a backup system while upgrading the primary system, users experience zero interruption."
  },
  {
    id: 3,
    question: "A systems administrator receives approval to patch WebServer01 during a 2:00 AM maintenance window. While connected, the admin notices DatabaseServer01 also has pending updates. Why must the admin refrain from patching DatabaseServer01 during this session?",
    options: [
      { 
        text: "Scope Control (change approval is restricted strictly to authorized target systems)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Change approval is NOT a blank check! Scope Control enforces that you only touch the specific machines approved by the board, preventing unexpected outages on other servers."
  },
  {
    id: 4,
    question: "An engineer upgrades a central firewall management server, but the management console loses connection to remote firewalls because the remote firewalls require a specific underlying software library version before accepting management commands. What technical change factor was overlooked?",
    options: [
      { 
        text: "System and Library Dependencies (Order of Operations)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Dependencies mean 'B must happen before A!' If system components rely on specific library versions, updating one out of order will break communication."
  },
  {
    id: 5,
    question: "A network administrator uses automated software to store router and switch configuration files in a central repository with date/time stamps. When a misconfigured access control list breaks traffic, the admin compares current settings against yesterday's file and instantly restores the working version. What control is used?",
    options: [
      { 
      ...
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not a configuration tracking tool." 
      },
      { 
    ...
    laymanExplanation: "Version Control tracks every line of configuration history over time! If a bad setting is entered, you can view the diff and roll back to yesterday's good config in seconds."
  },
  {
    id: 6,
    question: "Following a technical change that renumbers IP subnets and replaces core distribution switches, what mandatory technical post-implementation task ensures future troubleshooting teams have accurate network maps and operational procedures?",
    options: [
      { 
        text: "Updating Network Diagrams and Standard Operating Procedures (SOPs)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "A technical change isn't complete until documentation is updated! Updating network diagrams and SOPs ensures future technicians aren't troubleshooting blind."
  }
];
...
export default function TechnicalChangeManagementMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-amber-600 via-yellow-600 to-amber-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between gap-4">
        <div>
          <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 text-white font-extrabold text-xs backdrop-blur-md mb-2">
            <span>CompTIA Security+ SY0-701 • Section 1.3</span>
          </div>
          <h1 class="text-3xl font-extrabold tracking-tight">
            1.3 - Technical Change Management Practice ⚙️
          </h1>
          <p class="text-amber-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Allow Lists vs Deny Lists, HA Downtime Failover, Scope Control, System Dependencies, Configuration Version Control, and Network Diagrams!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 1.3 Technical Change Management for CompTIA Security+!'
