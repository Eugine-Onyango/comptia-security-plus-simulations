...
const rawQuestions = [
  {
    id: 1,
    question: "A company CEO digitally signs a million-dollar procurement contract. Later, during a legal dispute, the CEO attempts to claim that a rogue employee sent the contract without authorization. However, PKI digital signature logs mathematically prove the CEO's private key signed the document. What cryptographic property prevents the CEO from denying this action?",
    options: [
      { 
      ...
      { 
        text: "SQL Injection", 
        isCorrect: false, 
        whyWrong: "SQL Injection is a database vulnerability attack, not a cryptographic security property." 
      },
      { 
        text: "Typosquatting", 
        isCorrect: false, 
        whyWrong: "Typosquatting involves registering misspelled web domain names to trick users." 
      },
      { 
        text: "RF Jamming", 
        isCorrect: false, 
        whyWrong: "RF Jamming floods wireless frequencies with noise to disrupt radio signals." 
      }
    ],
    laymanExplanation: "Non-repudiation means 'There's no taking it back!' Cryptographically signing a contract proves you really signed it, so you cannot deny your actions later in court."
  },
  {
    id: 2,
    question: "Alice wants to send an official job offer letter ('You're hired, Bob') to Bob with non-repudiation and integrity. What exact steps does Alice take on her computer to create a valid digital signature?",
    options: [
      { 
        text: "Alice hashes the plaintext letter, then encrypts that hash using her Private Key", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Telnet transmits data unencrypted over port 23, providing zero cryptographic signatures." 
      },
      { 
    ...
    laymanExplanation: "Creating a Digital Signature is a 2-step recipe: 1) Hash the document to make a digital fingerprint, 2) Encrypt that fingerprint with YOUR PRIVATE KEY. Because only you have your private key, only you could have signed it!"
  },
  {
    id: 3,
    question: "Bob receives Alice's job offer letter along with her digital signature. How does Bob's computer verify both the origin (Authentication/Non-repudiation) and the message integrity?",
    options: [
      { 
        text: "Bob decrypts the digital signature using Alice's Public Key to get the hash, then compares it to his own calculated hash of the plaintext", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Vishing involves phone call voice phishing to trick victims." 
      },
      { 
        text: "HVAC thermal shutdown", 
        isCorrect: false, 
        whyWrong: "HVAC thermal shutdown turns off data center cooling systems to manage overheating." 
      },
      { 
    ...
    laymanExplanation: "To verify a signature: 1) Decrypt the signature with the sender's PUBLIC KEY to reveal the sender's hash. 2) Hash the message yourself. If both hashes match, the message is 100% genuine and unaltered!"
  },
  {
    id: 4,
    question: "An investigator calculates the SHA-256 hash of an 8.1 Megabyte electronic encyclopedia. If an attacker modifies just a single character somewhere inside the 8 million bytes of text, what happens to the output hash?",
    options: [
      { 
        text: "The output hash changes completely, detecting that data integrity was compromised", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "This is the 'Avalanche Effect' of cryptographic hashing! Changing even a single comma or letter in a massive 8MB encyclopedia changes the entire fingerprint hash, instantly flagging sneaky edits."
  },
  {
    id: 5,
    question: "A security analyst explains why a standalone SHA-256 hash digest guarantees Integrity but does NOT provide Non-repudiation. Why is a hash alone insufficient to prove origin?",
    options: [
      { 
        text: "A hash only shows if data changed, but does not associate the data with a specific individual's private identity", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not a cryptographic identity concept." 
      },
      { 
        text: "Disabling WPA3 enterprise encryption", 
        isCorrect: false, 
        whyWrong: "Disabling WPA3 affects Wi-Fi encryption, not cryptographic non-repudiation." 
      },
      { 
        text: "Forcing CPU thermal throttling", 
        isCorrect: false, 
        whyWrong: "Thermal throttling is a hardware heat response, not a cryptographic hashing limitation." 
      }
    ],
    laymanExplanation: "A hash is just a fingerprint of the file — it tells you IF the file changed, but NOT WHO touched it! You need a private key signature to link that file to a specific person."
  },
  {
    id: 6,
    question: "When a digital signature is applied to a message, it delivers a triple security guarantee to the recipient. Which three security outcomes are simultaneously achieved by a valid digital signature?",
    options: [
      { 
        text: "Integrity (message unedited), Authentication (proven sender identity), and Non-repudiation (sender cannot deny it)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Digital Signatures give you a 3-in-1 security combo! 1) Integrity (nobody edited the message), 2) Authentication (proves who sent it), 3) Non-repudiation (the sender can't claim 'It wasn't me!')."
  }
];
...
export default function NonRepudiationMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-amber-600 via-yellow-600 to-amber-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            1.2 - Non-repudiation Practice ✍️
          </h1>
          <p class="text-amber-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Proof of Origin, Proof of Integrity, Digital Signature Creation (Private Key) & Verification (Public Key), and Hashing Avalanche Effect!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 1.2 Non-repudiation for CompTIA Security+!'
