...
const rawQuestions = [
  {
    id: 1,
    question: "A healthcare provider issues laptops to travelling nurses. To protect patient health records from being accessed if a laptop is stolen from a vehicle, the IT team enables BitLocker across all drives. What mitigation technique is being utilized?",
    options: [
      { 
    ...
    laymanExplanation: "Full Disk Encryption (FDE) protects stolen hardware! Software like BitLocker or FileVault scrambles every single byte on the hard drive into unreadable code. Without the decryption key or TPM chip authentication, a thief cannot read your files."
  },
  {
    id: 2,
    question: "A company removes local Administrator privileges from all standard employee workstation accounts. When a user clicks a malicious email attachment, the malware fails to install because it lacks administrative permissions to modify system folders. What principle successfully mitigated the attack?",
    options: [
      { 
    ...
    laymanExplanation: "Least Privilege keeps damage contained! If users don't have admin powers for everyday tasks, malware launched in their session also doesn't get admin powers—stopping infections right in their tracks."
  },
  {
    id: 3,
    question: "When an employee connects their laptop to the corporate network, a Network Access Control (NAC) agent checks the device's OS patch level, EDR agent status, and host firewall configuration. If out of compliance, the device is isolated on a quarantine VLAN. What mitigation process is occurring?",
    options: [
      { 
        text: "Configuration Enforcement via Posture Assessment", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Posture Assessments act like a health checkup before entering a building! If your laptop is missing security patches or has its firewall turned off, the network quarantines it to a sandbox until it is updated."
  },
  {
    id: 4,
    question: "A critical zero-day vulnerability in an enterprise firewall OS is actively being exploited on the internet. Rather than waiting for the regular monthly patch cycle, the vendor releases a patch that IT immediately deploys. What type of update is this?",
    options: [
      { 
        text: "Emergency Out-of-Band Update", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Out-of-band patches are emergency fixes! When an active zero-day threat is loose, security teams cannot wait for the monthly schedule—they patch immediately out-of-band."
  },
  {
    id: 5,
    question: "An organization deploys software that ingests logs from firewalls, web servers, Active Directory, and endpoint EDR agents into a single console. The software uses a correlation engine to identify multi-stage attacks. What tool is being used?",
    options: [
      { 
        text: "SIEM (Security Information and Event Management) Collector", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "SIEMs combine clues from every computer! By putting firewall, server, and login logs together in one place, the SIEM correlation engine spots suspicious patterns across your entire network."
  },
  {
    id: 6,
    question: "An IT department is replacing 100 retired server magnetic hard drives. To guarantee that confidential corporate data can never be recovered from the old drives, the company contracts a vendor to run them through an industrial shredder. What policy is being executed?",
    options: [
      { 
        text: "Formal Media Decommissioning and Physical Destruction Policy", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Never throw old hard drives in the trash! Formal Decommissioning ensures retired storage drives are magnetically degaussed or physically shredded so dump-divers can never steal your old data."
  }
];
...
export default function MitigationTechniquesMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            2.5 - Mitigation Techniques Practice 🛠️
          </h1>
          <p class="text-amber-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Full Disk Encryption (FDE), Least Privilege, Posture Assessments, Emergency Out-of-Band Patching, SIEM Collectors, and Media Decommissioning!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 2.5 Mitigation Techniques for CompTIA Security+!'
