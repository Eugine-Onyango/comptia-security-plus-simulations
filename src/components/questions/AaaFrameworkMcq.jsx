...
const rawQuestions = [
  {
    id: 1,
    question: "A network router logs every administrator session, recording the exact logon time, commands executed, bytes transferred, and logoff time into a centralized syslog server. Which element of the AAA framework is being performed by this audit logging?",
    options: [
      { 
        text: "Accounting", 
        isCorrect: true, 
      ...
      { 
        text: "SQL Injection", 
        isCorrect: false, 
        whyWrong: "SQL Injection is a database vulnerability attack, not an AAA framework element." 
      },
      { 
        text: "Typosquatting", 
        isCorrect: false, 
    ...
    laymanExplanation: "Accounting is the audit receipt! It tracks resource usage — login time, logout time, data sent/received, and actions taken — so you have a complete record."
  },
  {
    id: 2,
    question: "An organization deploys 500 headless IoT devices across remote branches. Because these automated systems cannot type passwords, the security team installs digital certificates signed by the internal Certificate Authority (CA) on each device to authorize VPN connections. What method is used to authenticate these devices?",
    options: [
      { 
        text: "Certificate Authentication (Device Certificates signed by a trusted CA)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Telnet transmits unencrypted cleartext data over port 23, providing no certificate authentication." 
      },
      { 
    ...
    laymanExplanation: "Headless devices can't type passwords! Certificate authentication puts a digital ID card (signed by your organization's CA) directly onto the device so the network can verify it automatically."
  },
  {
    id: 3,
    question: "A sysadmin attempts to manage permissions for 5,000 employees by assigning individual file access rights directly to each user account one-by-one. The process quickly becomes unmanageable. What solution places an abstraction layer in the middle to streamline administration?",
    options: [
      { 
        text: "Using an Authorization Model (associating users to Roles or Groups)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Direct User-to-Resource permissions do NOT scale! Placing an Authorization Model (like a 'Shipping & Receiving' role) in the middle lets you grant permissions to the job role once, automatically covering everyone in that group."
  },
  {
    id: 4,
    question: "When logging into a secure portal, an employee first types their username 'jdoe', then enters a password and 6-digit authenticator code. Once validated, the system checks Active Directory to determine if 'jdoe' is allowed to edit financial ledgers. Which distinct AAA phases occurred in sequence?",
    options: [
      { 
        text: "Identification (username), Authentication (password/MFA), and Authorization (checking edit permissions)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "The 3 Steps: 1) Identification = 'Who are you claiming to be?' (username), 2) Authentication = 'Prove it!' (password/code), 3) Authorization = 'What are you allowed to do?' (permissions)."
  },
  {
    id: 5,
    question: "A company's Endpoint Management Server validates a laptop attempting to connect to the corporate network. How does the server verify that the laptop's client certificate is authentic and has not been forged?",
    options: [
      { 
        text: "By validating the digital signature of the trusted Certificate Authority (CA) on the certificate", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not a certificate validation process." 
      },
      { 
        text: "Disabling WPA3 enterprise encryption", 
        isCorrect: false, 
        whyWrong: "Disabling WPA3 affects Wi-Fi security, not CA digital signature validation." 
      },
      { 
        text: "Forcing CPU thermal throttling", 
        isCorrect: false, 
        whyWrong: "Thermal throttling is a hardware heat response, not a certificate validation technique." 
      }
    ],
    laymanExplanation: "The server checks the CA's digital stamp! Just like checking a official seal on a passport, the server validates the Certificate Authority's digital signature to make sure the device certificate isn't fake."
  },
  {
    id: 6,
    question: "An auditor asks why an enterprise network uses centralized TACACS+ and RADIUS servers instead of local device user databases. What core framework is centralized by these AAA servers?",
    options: [
      { 
        text: "AAA Framework (Centralized Identification, Authentication, Authorization, and Accounting)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "AAA Servers (TACACS+/RADIUS) centralize security management! Instead of setting up users on 500 separate routers, one AAA server handles Identification, Authentication, Authorization, and Accounting for everything."
  }
];
...
export default function AaaFrameworkMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-amber-600 via-yellow-600 to-amber-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            1.2 - AAA Framework Practice 🗝️
          </h1>
          <p class="text-amber-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Identification, Authentication, Authorization, Accounting, Certificate Authentication for Headless Systems, and Authorization Models!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 1.2 AAA Framework for CompTIA Security+!'
