...
const rawQuestions = [
  {
    id: 1,
    question: "A defense facility handles classified documents stamped with security labels (Top Secret, Secret, Confidential). The operating system enforces access strictly based on user clearance levels, preventing file creators from altering permissions or sharing files with lower-clearance users. What model is enforced?",
    options: [
      { 
        text: "MAC (Mandatory Access Control)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "MAC is top-secret military lockdown! Security labels (like Secret or Top Secret) are set by administrators, and regular users cannot change permissions or overshare files."
  },
  {
    id: 2,
    question: "An accountant creates a project spreadsheet on a shared drive and personally uses the operating system file properties tab to grant read-write permissions to a colleague. What access control model grants the file owner full discretion over sharing permissions?",
    options: [
      { 
    ...
    laymanExplanation: "DAC gives the file owner total control! If you create a document, DAC lets you decide who gets read or write access."
  },
  {
    id: 3,
    question: "An organization assigns network permissions based on employee job functions. When a new employee is hired as a 'Helpdesk Technician', adding their account to the 'Helpdesk_Staff' Active Directory group automatically grants all needed software rights. What model is used?",
    options: [
      { 
    ...
    laymanExplanation: "Role-Based Access Control (RBAC) gives access based on your job badge! Accountants get accounting tools, and IT techs get repair tools through group membership."
  },
  {
    id: 4,
    question: "A next-generation cloud gateway evaluates multiple dynamic parameters—user identity, current IP location, device security posture, time of day, and data classification—before deciding whether to grant access to sensitive records. What model is implemented?",
    options: [
      { 
    ...
    laymanExplanation: "ABAC looks at the whole context! Instead of checking just your name, ABAC checks your location, time of day, device health, and file sensitivity before opening the door."
  },
  {
    id: 5,
    question: "A network security appliance enforces a strict system rule that states: 'Web form submissions are only permitted between 9:00 AM and 5:00 PM when originating from an approved Chrome browser.' What access control model relies on these system-enforced condition rules?",
    options: [
      { 
      ...
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not Rule-based access control." 
      },
      { 
        text: "Disabling WPA3 enterprise encryption", 
        isCorrect: false, 
        whyWrong: "Disabling WPA3 affects Wi-Fi security, not Rule-based access control." 
      },
      { 
        text: "Forcing CPU thermal throttling", 
        isCorrect: false, 
        whyWrong: "Thermal throttling is a hardware heat response, not Rule-based access control." 
      }
    ],
    laymanExplanation: "Rule-Based Access Control follows system rules! Regardless of who you are, if the system rule says 'No access after 5 PM', the door stays locked."
  },
  {
    id: 6,
    question: "To protect sensitive R&D databases from off-hours tampering, a system administrator configures a policy that completely disables user logins to the database server between 8:00 PM and 6:00 AM on weekdays and all day on weekends. What policy feature is applied?",
    options: [
      { 
    ...
    laymanExplanation: "Time-of-Day Restrictions act like an automated night lock! They lock server doors after business hours so nobody can log in during late night hours."
  }
];
...
export default function AccessControlsMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-blue-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            4.6 - Access Controls Practice 🛡️
          </h1>
          <p class="text-blue-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Mandatory Access Control (MAC), Discretionary Access Control (DAC), Role-Based (RBAC), Rule-Based, Attribute-Based (ABAC), and Time-of-Day Restrictions!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-blue-50 border border-blue-200 rounded-2xl max-w-md mx-auto text-blue-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 4.6 Access Control Models for CompTIA Security+!'
