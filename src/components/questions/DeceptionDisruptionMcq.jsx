...
const rawQuestions = [
  {
    id: 1,
    question: "A security analyst configures an intentionally vulnerable server on an isolated network segment. The server contains fake database records and is designed solely to lure threat actors away from production systems while observing their attack techniques. What deception technology is deployed?",
    options: [
      { 
        text: "Honeypot", 
        isCorrect: true, 
      ...
      { 
        text: "SQL Injection", 
        isCorrect: false, 
        whyWrong: "SQL Injection is a web database attack technique, not a deception system." 
      },
      { 
        text: "Typosquatting", 
        isCorrect: false, 
    ...
    laymanExplanation: "A Honeypot is a virtual trap! It looks like a valuable target to attract bad guys, but it's isolated so security teams can watch how attackers operate without putting real data at risk."
  },
  {
    id: 2,
    question: "An enterprise security team constructs a complex decoy network complete with fake domain controllers, virtual workstations, decoy switches, and multiple honeypots to observe multi-stage cyberattacks. What deception environment is built?",
    options: [
      { 
        text: "Honeynet (a network of multiple decoy systems and honeypots)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Telnet transmits unencrypted cleartext data over port 23, providing no honeynet deception." 
      },
      { 
    ...
    laymanExplanation: "A Honeynet is a whole fake neighborhood! Instead of just one decoy server, a Honeynet simulates entire networks of servers, switches, and workstations to watch how attackers move laterally."
  },
  {
    id: 3,
    question: "A sysadmin places a document named 'Executive_Passwords_2026.docx' on an internal network file share. The file contains dummy credentials and triggers an instant high-priority SIEM alert the second any user attempts to open or read it. What bait is deployed?",
    options: [
      { 
        text: "Honeyfile (a decoy file acting as a bear trap)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "A Honeyfile is a digital bear trap! It looks like a tempting file (like 'passwords.txt') so when a hacker clicks it, an instant security alarm fires off."
  },
  {
    id: 4,
    question: "A cloud security engineer embeds fake AWS API access keys into a GitHub repository. The keys do not grant access to any real cloud infrastructure, but automatically notify the SOC team if an attacker attempts to use them. What traceable data element is used?",
    options: [
      { 
        text: "Honeytoken (decoy credentials/data used to track malicious actors)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Honeytokens are fake credentials, email addresses, or API keys! They don't unlock real systems, but the moment someone tries to use them, you instantly catch the intruder."
  },
  {
    id: 5,
    question: "A company seeds a unique, fake email address ('decoy_admin@company.com') inside its customer database. Weeks later, security personnel observe spam emails targeting that exact address. What does this honeytoken reveal?",
    options: [
      { 
        text: "Proof that the customer database was breached and exfiltrated", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not a breach detection indicator." 
      },
      { 
    ...
    laymanExplanation: "Honeytokens act like tracer dye! If a fake email address stored inside your database suddenly starts getting spammed, you know for sure that database was leaked."
  },
  {
    id: 6,
    question: "A SOC analyst receives a SIEM alert indicating network traffic is flowing to an internal honeypot server. Why is any traffic targeting a honeypot classified as an immediate security threat?",
    options: [
      { 
        text: "Honeypots host no legitimate business services, so any connection attempt is unauthorized by default", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Honeypots have ZERO real work! Because no real employee or program ever has a reason to visit a honeypot, ANY traffic hitting it is instantly flagged as suspicious or malicious."
  }
];
...
export default function DeceptionDisruptionMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-amber-600 via-yellow-600 to-amber-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            1.2 - Deception & Disruption Practice 🍯
          </h1>
          <p class="text-amber-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Honeypots, Honeynets, Honeyfiles, Honeytokens (API credentials, fake emails, database records), and TTP Threat Intelligence!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 1.2 Deception & Disruption for CompTIA Security+!'
