...
const rawQuestions = [
  {
    id: 1,
    question: "A security administrator updates the organization's password policy to prevent offline GPU brute-force attacks. Instead of forcing short, hard-to-remember character strings, the policy mandates 16-character multi-word passphrases with mixed cases and symbols. What property is maximized by increasing length and character variation?",
    options: [
      { 
    ...
    laymanExplanation: "Entropy measures randomness and unpredictability! Longer passphrases have high entropy, making it mathematically impossible for computer brute-force tools to guess."
  },
  {
    id: 2,
    question: "Employees at a firm bypass mandatory 60-day password expiration rules by switching back and forth between 'Summer2025!' and 'Winter2025!'. What Active Directory domain setting must the administrator configure to prevent users from reusing recent passwords?",
    options: [
      { 
    ...
    laymanExplanation: "Password History remembers your old passwords! If set to 10, the system remembers your last 10 passwords and blocks you from recycling old ones."
  },
  {
    id: 3,
    question: "An IT department wants employees to use unique 24-character passwords for every cloud portal without writing them on sticky notes. What solution securely stores all credentials in an encrypted database with centralized enterprise recovery options?",
    options: [
      { 
        text: "Enterprise Password Manager", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Password Managers act like a digital vault! You remember ONE strong master key, and the password manager automatically generates and autofills distinct complex passwords for every website."
  },
  {
    id: 4,
    question: "To eliminate credential theft from phishing websites and keystroke loggers, an enterprise equips workstations with FIDO2 WebAuthn hardware security keys and biometric sensors so users can log in without typing passwords. What model is implemented?",
    options: [
      { 
    ...
    laymanExplanation: "Passwordless means no text passwords to leak or steal! You log in using hardware tokens or biometrics directly, eliminating password breaches."
  },
  {
    id: 5,
    question: "To enforce least privilege and reduce standing administrative accounts, an enterprise PAM system grants technicians elevated root access for 2 hours only upon approving a ticket request, automatically revoking rights when the window closes. What concept is enforced?",
    options: [
      { 
        text: "Just-in-Time (JIT) Permissions", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not JIT permissions." 
      },
      { 
        text: "Disabling WPA3 enterprise encryption", 
        isCorrect: false, 
        whyWrong: "Disabling WPA3 affects Wi-Fi security, not JIT permissions." 
      },
      { 
        text: "Forcing CPU thermal throttling", 
        isCorrect: false, 
        whyWrong: "Thermal throttling is a hardware heat response, not JIT permissions." 
      }
    ],
    laymanExplanation: "Just-in-Time (JIT) permissions grant temporary admin keys only when you need them! Once the job is done, administrative privileges disappear so hackers can't hijack standing admin accounts."
  },
  {
    id: 6,
    question: "A system administrator needs temporary SSH access to a production database server. Instead of revealing the master root password, the PAM vault generates a single-use, auto-expiring temporary password that self-destructs after the session ends. What feature protects the primary credential?",
    options: [
      { 
        text: "Password Vaulting with Ephemeral Credentials", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Password Vaulting with Ephemeral Credentials locks master passwords inside a vault and hands out temporary 'guest passes' (ephemeral keys) that expire right after use."
  }
];
...
export default function PasswordSecurityMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-blue-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            4.6 - Password Security Practice 🔑
          </h1>
          <p class="text-blue-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Password Entropy, Password History, Enterprise Managers, Passwordless Auth, Just-In-Time (JIT) Permissions, and Ephemeral Credentials!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-blue-50 border border-blue-200 rounded-2xl max-w-md mx-auto text-blue-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 4.6 Password Security for CompTIA Security+!'
