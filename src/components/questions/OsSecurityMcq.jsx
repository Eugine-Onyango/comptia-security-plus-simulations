...
const rawQuestions = [
  {
    id: 1,
    question: "A Windows system administrator needs to manage 5,000 user accounts, workstations, printers, and file permissions from a single centralized database so employees can log in with one set of credentials across the company. What service is utilized?",
    options: [
      { 
    ...
    laymanExplanation: "Active Directory (AD) is the phonebook and key master for Windows networks! It holds all user accounts, computers, and access rights in one central location so you can log into any company PC with your username."
  },
  {
    id: 2,
    question: "A security team needs to enforce a 15-minute screen lockout timer, disable the Command Prompt, and run an automated login script across 2,000 domain-joined Windows PCs. What central administration tool should be configured?",
    options: [
      { 
    ...
    laymanExplanation: "Group Policy Objects (GPOs) push settings to thousands of computers at once! Instead of walking to 2,000 desks to lock down settings, GPOs deploy security rules automatically over the network."
  },
  {
    id: 3,
    question: "A Linux administrator wants to protect a web server so that even if an attacker exploits a web application vulnerability, the web process cannot access sensitive system files like /etc/shadow, overriding standard owner permissions. What kernel security architecture provides this control?",
    options: [
      { 
        text: "SELinux (Security-Enhanced Linux) Mandatory Access Control", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "SELinux adds supercharged Mandatory Access Control (MAC)! Normal Linux relies on Discretionary Access Control (DAC) where file owners decide permissions. SELinux puts strict kernel rules in place that even file owners cannot bypass."
  },
  {
    id: 4,
    question: "Traditional Linux permissions allow file owners to grant read/write permissions to any user (Discretionary Access Control - DAC). How does SELinux improve operating system security compared to traditional Linux permissions?",
    options: [
      { 
        text: "It enforces strict system-wide Mandatory Access Control (MAC) rules that file owners cannot override", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "SELinux takes control away from user mistakes! Under traditional DAC, a user could accidentally share a sensitive file. Under SELinux MAC, administrator rules strictly dictate what apps and users are allowed to do."
  },
  {
    id: 5,
    question: "An attacker compromises a Linux mail daemon service running SELinux. The attacker attempts to read core system configuration files, but the kernel blocks access with 'Permission Denied' despite the daemon running as root. What security benefit was demonstrated?",
    options: [
      { 
        text: "Confining the breach scope / Blast Radius via Least Privilege", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not OS breach confinement." 
      },
      { 
        text: "Disabling WPA3 enterprise encryption", 
        isCorrect: false, 
        whyWrong: "Disabling WPA3 affects Wi-Fi security, not OS kernel confinement." 
      },
      { 
        text: "Forcing CPU thermal throttling", 
        isCorrect: false, 
        whyWrong: "Thermal throttling is a hardware heat response, not OS kernel confinement." 
      }
    ],
    laymanExplanation: "SELinux confines hackers to a tiny sandbox! Even if a hacker hacks into one service, SELinux prevents them from touching anything else on the server, drastically limiting the damage."
  },
  {
    id: 6,
    question: "A help desk technician notices that newly imaged Windows laptops automatically receive corporate firewall rules, USB block policies, and password requirements as soon as they join the domain. What Active Directory mechanism delivered these rules?",
    options: [
      { 
        text: "Domain Group Policy Objects (GPOs)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Domain GPOs automate setup! Joining a laptop to Active Directory automatically pulls down all security settings, saving IT technicians from configuring machines by hand."
  }
];
...
export default function OsSecurityMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-blue-200/50 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-extrabold tracking-tight">
            4.5 - Operating System Security Practice 💻
          </h1>
          <p class="text-blue-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering Active Directory (AD), Group Policy Objects (GPOs), Security-Enhanced Linux (SELinux Mandatory Access Control - MAC), and Blast Radius Confinement!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-blue-50 border border-blue-200 rounded-2xl max-w-md mx-auto text-blue-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 4.5 Operating System Security for CompTIA Security+!'
