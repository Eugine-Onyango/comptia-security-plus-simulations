...
const rawQuestions = [
  {
    id: 1,
    question: "An organization replaces its traditional perimeter firewall security model after discovering that attackers who breached the outer firewall moved laterally across internal servers without resistance. The new security architecture assumes no user or device is inherently trustworthy, requiring continuous authentication for every internal connection. Which security architecture is implemented?",
    options: [
      { 
      ...
      { 
        text: "SQL Injection", 
        isCorrect: false, 
        whyWrong: "SQL Injection is a web database attack technique, not a network security architecture." 
      },
      { 
        text: "Typosquatting", 
        isCorrect: false, 
    ...
    laymanExplanation: "Zero Trust is built on the rule: 'Never Trust, Always Verify!' Instead of assuming internal network users are safe, Zero Trust treats EVERY connection (inside or outside) as untrusted until verified."
  },
  {
    id: 2,
    question: "In a Zero Trust architecture diagram, Component A evaluates access policy rules alongside risk indicators to make a Grant or Deny access decision. Component B then generates credentials and instructs Component C (located in the Data Plane) to allow or terminate the actual user session. Which components fulfill these roles?",
    options: [
      { 
        text: "Policy Engine (Component A), Policy Administrator (Component B), and Policy Enforcement Point (Component C)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Telnet transmits unencrypted cleartext data over port 23, providing no Zero Trust policy architecture." 
      },
      { 
    ...
    laymanExplanation: "The Zero Trust Decision Trio: 1) Policy Engine decides (Grant/Deny), 2) Policy Administrator passes the order & creates tokens, 3) Policy Enforcement Point (PEP) stands at the gate to open or shut the network door!"
  },
  {
    id: 3,
    question: "A network architecture separates operations into functional planes. Router functions that handle frame forwarding, packet switching, NAT translation, and hardware traffic encryption operate on Plane X, while routing tables and control rules operate on Plane Y. What are these two planes?",
    options: [
      { 
        text: "Data Plane (Plane X - handles traffic processing) and Control Plane (Plane Y - manages policy/routing rules)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Data Plane does the heavy lifting (moving frames, packets, NAT)! Control Plane acts as the brain (setting routing tables, session rules, and forwarding policies)."
  },
  {
    id: 4,
    question: "An employee attempts to access sensitive payroll files at 2:00 AM from an unknown IP address in another country using an unmanaged personal laptop. The access gateway dynamically prompts for step-up MFA and restricts access based on device health, user location, and connection type. What Zero Trust concept evaluates these dynamic variables?",
    options: [
      { 
        text: "Adaptive Identity (Contextual Risk Assessment)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Adaptive Identity looks at the WHOLE picture! Instead of checking a password alone, it analyzes location, device health, time of day, and IP address to adjust trust dynamically."
  },
  {
    id: 5,
    question: "A firewall gateway acts as the Policy Enforcement Point (PEP) between an untrusted network zone and a trusted database server. If the Policy Decision Point (PDP) revokes access during an active session, what action does the PEP take on the data plane?",
    options: [
      { 
        text: "The PEP immediately blocks or terminates the active connection to the enterprise resource", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
        ...
        isCorrect: false, 
        whyWrong: "Gigabit switch latency is a network timing metric, not a Policy Enforcement Point action." 
      },
      { 
    ...
    laymanExplanation: "The PEP is the bouncer at the door! When the decision engine says 'Revoke access', the PEP instantly severs the live network connection to protect the server."
  },
  {
    id: 6,
    question: "An organization segments its internal network into isolated security micro-zones (such as Marketing, Accounting, IT, and External VPNs). By restricting direct traffic flows between zones, the organization limits an attacker's ability to move laterally across systems. What key objective of Zero Trust is achieved?",
    options: [
      { 
        text: "Threat Scope Reduction (reducing blast radius and entry points)", 
        isCorrect: true, 
        whyWrong: "" 
      },
      { 
    ...
    laymanExplanation: "Threat Scope Reduction divides the network into small security compartments! If an attacker compromises one compartment, micro-segmentation prevents them from spreading across the rest of the company."
  }
];
...
export default function ZeroTrustMcq({ onBack }) {
      ...
      <div class="bg-gradient-to-r from-amber-600 via-yellow-600 to-amber-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between gap-4">
        <div>
          <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 text-white font-extrabold text-xs backdrop-blur-md mb-2">
            <span>CompTIA Security+ SY0-701 • Section 1.2</span>
          </div>
          <h1 class="text-3xl font-extrabold tracking-tight">
            1.2 - Zero Trust Architecture Practice 🛡️⚡
          </h1>
          <p class="text-amber-100 font-medium text-sm mt-1">
            CompTIA Security+ scenario questions covering 'Never Trust, Always Verify', Data Plane vs Control Plane, PDP (Policy Engine & Administrator), PEP (Policy Enforcement Point), Adaptive Identity, and Threat Scope Reduction!
          </p>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered 1.2 Zero Trust for CompTIA Security+!'
