...
const questions = [
  {
    id: 1,
    question: "Which hardware cryptoprocessor chip is integrated directly onto a computer motherboard to protect BitLocker encryption keys?",
    options: [
      { text: "TPM (Trusted Platform Module)", isCorrect: true },
      { text: "RAM (Random Access Memory)", isCorrect: false },
      { text: "GPU (Graphics Processing Unit)", isCorrect: false }
    ],
    explanation: "TPM (Trusted Platform Module) is the dedicated hardware security chip integrated into PC motherboards!"
  },
  {
    id: 2,
    question: "A cloud bank needs a high-end standalone hardware appliance to manage millions of digital keys and offload heavy crypto math. What is it?",
    options: [
      { text: "USB Flash Drive", isCorrect: false },
      { text: "HSM (Hardware Security Module)", isCorrect: true },
      { text: "DVD-ROM", isCorrect: false }
    ],
    explanation: "HSM (Hardware Security Module) is a dedicated high-performance enterprise vault appliance!"
  },
  {
    id: 3,
    question: "Without hardware accelerators, calculating millions of SSL/TLS and AES formulas in pure software exhausts which main computer brain?",
    options: [
      { text: "CPU (Central Processing Unit)", isCorrect: true },
      { text: "HDD (Hard Disk Drive)", isCorrect: false },
      { text: "SSD (Solid State Drive)", isCorrect: false }
    ],
    explanation: "The CPU (Central Processing Unit) is the main processor whose workload is offloaded by TPM & HSM modules!"
  },
  {
    id: 4,
    question: "Where is immutable boot verification code stored inside a TPM chip that cannot be altered by malware?",
    options: [
      { text: "ROM (Read-Only Memory / boot ROM)", isCorrect: true },
      { text: "Web Cache", isCorrect: false },
      { text: "Cloud Backup", isCorrect: false }
    ],
    explanation: "ROM (Read-Only Memory) stores permanent firmware like the TPM boot ROM!"
  },
  {
    id: 5,
    question: "Which hardware-accelerated symmetric algorithm is executed directly inside silicon chips for fast bulk encryption?",
    options: [
      { text: "AES (Advanced Encryption Standard)", isCorrect: true },
      { text: "ASCII", isCorrect: false },
      { text: "HTTP", isCorrect: false }
    ],
    explanation: "AES (Advanced Encryption Standard) is the industry-standard symmetric encryption algorithm accelerated in hardware!"
  }
];

export default function EncryptionTechQuiz() {
  ...
  return (
    <div class="space-y-6">
      ...
      {/* Top Banner */}
      <div class="bg-gradient-to-r from-amber-500 to-orange-600 rounded-3xl p-6 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between">
        <div>
          <span class="text-xs font-extrabold uppercase tracking-widest text-amber-100">Encryption Technologies Challenge</span>
          <h3 class="text-2xl font-extrabold">Master TPM, HSM, CPU, AES & ROM! 🛡️</h3>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered TPM, HSM, CPU, AES, SSL, TLS, SSH, & ROM!'
              : '👍 Great job! Review the hardware toys and try again for 100%!'}
          </div>
