import React, { useState } from 'react';
import { ArrowLeft, Award, Sliders, Globe } from 'lucide-react';
import CisOwaspSim from './CisOwaspSim';
import IsoNistSim from './IsoNistSim';
import SecurityStandardsQuiz from './SecurityStandardsQuiz';
import { sounds } from '../../utils/audio';

export default function SecurityStandardsSim({ onBack }) {
  const [activeTab, setActiveTab] = useState('cis_owasp');

  const handleTabChange = (tab) => {
    sounds.playPop();
    setActiveTab(tab);
  };

  return (
    <div class="max-w-6xl mx-auto px-4 py-8 space-y-8">
      
      {/* Navigation Breadcrumb */}
      <div class="flex items-center justify-between">
        <button
          onClick={onBack}
          class="flex items-center gap-2 px-3.5 py-2 bg-white hover:bg-sky-100 text-sky-900 border-2 border-sky-200 rounded-2xl font-bold text-sm transition-all shadow-sm active:scale-95"
        >
          <ArrowLeft class="w-4 h-4" />
          <span>Back to Domain 5.0</span>
        </button>

        <span class="px-3 py-1 bg-indigo-100 text-indigo-950 rounded-full font-extrabold text-xs">
          Topic 5.1 • Security Standards (ISO, IEC, NIST SP, CIS, OWASP)
        </span>
      </div>

      {/* Title Header */}
      <div class="bg-white rounded-3xl p-6 sm:p-8 border-4 border-indigo-300 shadow-md space-y-2">
        <div class="flex items-center gap-3">
          <span class="text-3xl">⚙️</span>
          <div>
            <h1 class="text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tight">
              5.1 - Security Standards
            </h1>
            <p class="text-slate-600 font-medium text-sm sm:text-base">
              Explore ISO/IEC 27001 ISMS certification frameworks, NIST Special Publications (SP 800-53), Center for Internet Security (CIS) system benchmarks, and OWASP Top 10 web security controls!
            </p>
          </div>
        </div>
      </div>

      {/* Tabs Navigation */}
      <div class="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
        
        <button
          onClick={() => handleTabChange('cis_owasp')}
          class={`px-4 py-3 rounded-2xl font-extrabold text-xs sm:text-sm whitespace-nowrap transition-all flex items-center gap-2 border-2 ${
            activeTab === 'cis_owasp'
              ? 'bg-indigo-600 border-indigo-700 text-white shadow-md shadow-indigo-200 scale-105'
              : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300'
          }`}
        >
          <Sliders class="w-4 h-4" />
          <span>CIS Hardening & OWASP Web Radar (CIS, OWASP)</span>
        </button>

        <button
          onClick={() => handleTabChange('iso_nist')}
          class={`px-4 py-3 rounded-2xl font-extrabold text-xs sm:text-sm whitespace-nowrap transition-all flex items-center gap-2 border-2 ${
            activeTab === 'iso_nist'
              ? 'bg-purple-600 border-purple-700 text-white shadow-md shadow-purple-200 scale-105'
              : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300'
          }`}
        >
          <Globe class="w-4 h-4" />
          <span>ISO/IEC 27001 ISMS & NIST SP (ISO, IEC, NIST SP)</span>
        </button>

        <button
          onClick={() => handleTabChange('quiz')}
          class={`px-4 py-3 rounded-2xl font-extrabold text-xs sm:text-sm whitespace-nowrap transition-all flex items-center gap-2 border-2 ${
            activeTab === 'quiz'
              ? 'bg-emerald-500 border-emerald-600 text-white shadow-md shadow-emerald-200 scale-105'
              : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300'
          }`}
        >
          <Award class="w-4 h-4" />
          <span>Security Standards Quiz 🎮</span>
        </button>

      </div>

      {/* Tab Content Display */}
      <div class="bg-white rounded-3xl p-6 sm:p-8 border-2 border-slate-200 shadow-sm">
        {activeTab === 'cis_owasp' && <CisOwaspSim />}
        {activeTab === 'iso_nist' && <IsoNistSim />}
        {activeTab === 'quiz' && <SecurityStandardsQuiz />}
      </div>

    </div>
  );
}
