...
const questions = [
  {
    id: 1,
    question: "Which step of the AAA framework verifies WHO you claim to be by checking your password or fingerprint?",
    options: [
      { text: "Authentication (Identity Check)", isCorrect: true },
      { text: "Authorization", isCorrect: false },
      { text: "Accounting", isCorrect: false }
    ],
    explanation: "Authentication is the first A that checks who you claim to be!"
  },
  {
    id: 2,
    question: "Which step of the AAA framework checks WHAT permissions, files, or rooms you are allowed to access?",
    options: [
      { text: "Authentication", isCorrect: false },
      { text: "Authorization (Permission Ticket)", isCorrect: true },
      { text: "Accounting", isCorrect: false }
    ],
    explanation: "Authorization is the second A that grants or restricts specific access rights!"
  },
  {
    id: 3,
    question: "Which step of the AAA framework logs audit trails, timestamps, and user actions?",
    options: [
      { text: "Authentication", isCorrect: false },
      { text: "Authorization", isCorrect: false },
      { text: "Accounting (Audit Log Trail)", isCorrect: true }
    ],
    explanation: "Accounting records logs and tracks user actions for auditing!"
  },
  {
    id: 4,
    question: "In federated Single Sign-On, which trusted entity verifies the user once so multiple member apps accept them?",
    options: [
      { text: "CA (Central Authority)", isCorrect: true },
      { text: "SSD", isCorrect: false },
      { text: "CPU", isCorrect: false }
    ],
    explanation: "The Central Authority (CA) verifies the user's identity once in federated authentication!"
  },
  {
    id: 5,
    question: "When connecting to corporate AAA login systems from public Wi-Fi, what encrypts your entire traffic stream?",
    options: [
      { text: "VPN (Virtual Private Network)", isCorrect: true },
      { text: "USB", isCorrect: false },
      { text: "ROM", isCorrect: false }
    ],
    explanation: "A VPN (Virtual Private Network) provides an encrypted network tunnel for remote connections!"
  }
];

export default function AaaQuiz() {
  ...
  return (
    <div class="space-y-6">
      ...
      <div class="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-3xl p-6 text-white shadow-lg shadow-indigo-200/50 flex items-center justify-between">
        <div>
          <span class="text-xs font-extrabold uppercase tracking-widest text-indigo-100">AAA Framework Challenge</span>
          <h3 class="text-2xl font-extrabold">Master AAA, CA, & VPN! 🪪🎟️🧾</h3>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-indigo-50 border border-indigo-200 rounded-2xl max-w-md mx-auto text-indigo-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered Authentication, Authorization, Accounting, CA, & VPN!'
