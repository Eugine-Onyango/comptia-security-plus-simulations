...
import { ArrowLeft, Award, Users, Bot } from 'lucide-react';
import CcbSopSim from './CcbSopSim';
import SoarPlaybookSim from './SoarPlaybookSim';
import SecurityProceduresQuiz from './SecurityProceduresQuiz';
import { sounds } from '../../utils/audio';

export default function SecurityProceduresSim({ onBack }) {
  const [activeTab, setActiveTab] = useState('ccb_sop');
    ...
    <div class="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      <div class="flex items-center justify-between">
        ...
        <span class="px-3 py-1 bg-purple-100 text-purple-950 rounded-full font-extrabold text-xs">
          Topic 5.1 • Security Procedures (SOP, CCB, SOAR)
        </span>
      </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-slate-600 font-medium text-sm sm:text-base">
              Explore Standard Operating Procedures (SOP), Change Control Board (CCB) ticket voting, and Security Orchestration, Automation, and Response (SOAR) playbook execution!
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
        ...
        <button
          onClick={() => handleTabChange('ccb_sop')}
        ...
        >
          <Users class="w-4 h-4" />
          <span>CCB Board & SOP Checklist (CCB, SOP)</span>
        </button>

        <button
          onClick={() => handleTabChange('soar')}
          class={`px-4 py-3 rounded-2xl font-extrabold text-xs sm:text-sm whitespace-nowrap transition-all flex items-center gap-2 border-2 ${
            activeTab === 'soar'
        ...
        >
          <Bot class="w-4 h-4" />
          <span>SOAR Automated Playbook Engine (SOAR)</span>
        </button>
        ...
        >
          <Award class="w-4 h-4" />
          <span>Security Procedures Quiz 🎮</span>
        </button>
      ...
      <div class="bg-white rounded-3xl p-6 sm:p-8 border-2 border-slate-200 shadow-sm">
        {activeTab === 'ccb_sop' && <CcbSopSim />}
        {activeTab === 'soar' && <SoarPlaybookSim />}
        {activeTab === 'quiz' && <SecurityProceduresQuiz />}
      </div>
