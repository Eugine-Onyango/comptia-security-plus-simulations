...
import { ArrowLeft, Award, Search, UserCheck } from 'lucide-react';
import DpiaScannerSim from './DpiaScannerSim';
import DpoPrivacyDispatcherSim from './DpoPrivacyDispatcherSim';
import PrivacyQuiz from './PrivacyQuiz';
import { sounds } from '../../utils/audio';

export default function PrivacySim({ onBack }) {
  const [activeTab, setActiveTab] = useState('dpia');
    ...
    <div class="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      <div class="flex items-center justify-between">
        ...
        <span class="px-3 py-1 bg-purple-100 text-purple-950 rounded-full font-extrabold text-xs">
          Topic 5.4 • Privacy (GDPR, EU, IP, DPO, PIA, DPIA, PHI, PII)
        </span>
      </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-slate-600 font-medium text-sm sm:text-base">
              Explore Data Privacy Frameworks: General Data Protection Regulation (GDPR), European Union (EU), IP address classification, Data Protection Officer (DPO), Privacy Impact Assessments (PIA & DPIA), PHI, and PII!
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
        ...
        >
          ...
          <span>DPIA High-Risk Scanner (PIA, DPIA, GDPR, EU, IP)</span>
        </button>

        <button
          onClick={() => handleTabChange('dpo')}
          class={`px-4 py-3 rounded-2xl font-extrabold text-xs sm:text-sm whitespace-nowrap transition-all flex items-center gap-2 border-2 ${
            activeTab === 'dpo'
        ...
        >
          ...
          <span>DPO Privacy Ticket Dispatcher (DPO, PII, PHI)</span>
        </button>
      ...
      <div class="bg-white rounded-3xl p-6 sm:p-8 border-2 border-slate-200 shadow-sm">
        {activeTab === 'dpia' && <DpiaScannerSim />}
        {activeTab === 'dpo' && <DpoPrivacyDispatcherSim />}
        {activeTab === 'quiz' && <PrivacyQuiz />}
      </div>
