...
import { ArrowLeft, Award, Sparkles, Wifi } from 'lucide-react';
import DataMaskingSim from './DataMaskingSim';
import TcpSteganographySim from './TcpSteganographySim';
import ObfuscationQuiz from './ObfuscationQuiz';
import { sounds } from '../../utils/audio';

export default function ObfuscationSim({ onBack }) {
    ...
    <div class="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      <div class="flex items-center justify-between">
        ...
        <span class="px-3 py-1 bg-amber-100 text-amber-950 rounded-full font-extrabold text-xs">
          Topic 1.4 • Obfuscation (PII, SSN, TCP)
        </span>
      </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-slate-600 font-medium text-sm sm:text-base">
              Learn how to mask sensitive PII and SSN data for customer support, and how TCP network steganography hides secret messages inside packet headers!
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
        ...
        >
          <Wifi class="w-4 h-4" />
          <span>TCP Steganography (TCP)</span>
        </button>
      ...
      <div class="bg-white rounded-3xl p-6 sm:p-8 border-2 border-slate-200 shadow-sm">
        {activeTab === 'masking' && <DataMaskingSim />}
        {activeTab === 'steganography' && <TcpSteganographySim />}
        {activeTab === 'quiz' && <ObfuscationQuiz />}
      </div>
