...
import { ArrowLeft, Award, Calculator, Scale } from 'lucide-react';
import RiskMathSim from './RiskMathSim';
import RiskRoiSim from './RiskRoiSim';
import RiskAnalysisQuiz from './RiskAnalysisQuiz';
import { sounds } from '../../utils/audio';

export default function RiskAnalysisSim({ onBack }) {
    ...
    <div class="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      <div class="flex items-center justify-between">
        ...
        <span class="px-3 py-1 bg-indigo-100 text-indigo-950 rounded-full font-extrabold text-xs">
          Topic 5.2 • Risk Analysis (SLE, ARO, ALE, AV, EF)
        </span>
      </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-slate-600 font-medium text-sm sm:text-base">
              Explore Quantitative Risk Formulas: Single Loss Expectancy (SLE), Annualized Rate of Occurrence (ARO), Annualized Loss Expectancy (ALE), Asset Value (AV), and Exposure Factor (EF)!
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
        ...
        >
          ...
          <span>Risk Math Calculator (SLE, ARO, ALE, AV, EF)</span>
        </button>
        ...
        >
          <Scale class="w-4 h-4" />
          <span>Safeguard ROI Analyzer (ALE vs Cost)</span>
        </button>
      ...
      <div class="bg-white rounded-3xl p-6 sm:p-8 border-2 border-slate-200 shadow-sm">
        {activeTab === 'math' && <RiskMathSim />}
        {activeTab === 'roi' && <RiskRoiSim />}
        {activeTab === 'quiz' && <RiskAnalysisQuiz />}
      </div>
