...
const questions = [
  {
    id: 1,
    question: "What does MD5 stand for in cryptographic hashing?",
    options: [
      { text: "Message Digest algorithm 5", isCorrect: true },
      { text: "Main Drive 5", isCorrect: false },
      { text: "Maximum Data 5", isCorrect: false }
    ],
    explanation: "MD5 stands for Message Digest algorithm 5!"
  },
  {
    id: 2,
    question: "Why is MD5 deprecated in modern security standards?",
    options: [
      { text: "It is vulnerable to collision attacks where different files produce identical hashes", isCorrect: true },
      { text: "It requires too much RAM", isCorrect: false },
      { text: "It only works on Windows 95", isCorrect: false }
    ],
    explanation: "MD5 is deprecated because collision attacks allow forged files to share identical MD5 hashes!"
  },
  {
    id: 3,
    question: "What is a Hash Collision?",
    options: [
      { text: "When two different inputs produce the exact same hash output", isCorrect: true },
      { text: "When a hard drive crashes", isCorrect: false },
      { text: "When a Wi-Fi router loses power", isCorrect: false }
    ],
    explanation: "A collision occurs when two distinct inputs yield the identical hash result!"
  },
  {
    id: 4,
    question: "What is the core property of a cryptographic hash function?",
    options: [
      { text: "It is a one-way irreversible function", isCorrect: true },
      { text: "It can be easily reversed to recover the original text", isCorrect: false },
      { text: "It changes output length based on input file size", isCorrect: false }
    ],
    explanation: "Cryptographic hashes are one-way functions that produce fixed-length outputs!"
  },
  {
    id: 5,
    question: "Which collision-resistant hashing algorithm replaces deprecated MD5 in modern digital signatures?",
    options: [
      { text: "SHA-256", isCorrect: true },
      { text: "ZIP", isCorrect: false },
      { text: "PNG", isCorrect: false }
    ],
    explanation: "SHA-256 is the recommended collision-resistant standard!"
  }
];

export default function HashingQuiz() {
  ...
  return (
    <div class="space-y-6">
      ...
      {/* Top Banner */}
      <div class="bg-gradient-to-r from-amber-500 to-rose-600 rounded-3xl p-6 text-white shadow-lg shadow-amber-200/50 flex items-center justify-between">
        <div>
          <span class="text-xs font-extrabold uppercase tracking-widest text-amber-100">Hashing Challenge</span>
          <h3 class="text-2xl font-extrabold">Master MD5 & Hash Collisions! 🧬🥑</h3>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 border-2 border-slate-200 shadow-sm text-center space-y-6 animate-fadeIn">
          ...
          <div class="p-4 bg-amber-50 border border-amber-200 rounded-2xl max-w-md mx-auto text-amber-950 text-sm font-semibold">
            {score === questions.length 
              ? '🌟 PERFECT SCORE! You have mastered MD5, Hashing, & Collision Attacks!'
              : '👍 Great job! Review the Hash Blender toys and try again for 100%!'}
          </div>
