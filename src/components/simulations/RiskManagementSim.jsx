...
import { ArrowLeft, Award, RotateCw, Activity } from 'lucide-react';
import RmfLifecycleSim from './RmfLifecycleSim';
import KriRadarSim from './KriRadarSim';
import RiskManagementQuiz from './RiskManagementQuiz';
import { sounds } from '../../utils/audio';

export default function RiskManagementSim({ onBack }) {
    ...
    <div class="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      <div class="flex items-center justify-between">
        ...
        <span class="px-3 py-1 bg-indigo-100 text-indigo-950 rounded-full font-extrabold text-xs">
          Topic 5.2 • Risk Management (RMF, KRI)
        </span>
      </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-slate-600 font-medium text-sm sm:text-base">
              Explore the NIST Risk Management Framework (RMF 7-step lifecycle) and Key Risk Indicators (KRI early warning gauges)!
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
        ...
        >
          ...
          <span>NIST RMF 7-Step Lifecycle Engine (RMF)</span>
        </button>

        <button
          onClick={() => handleTabChange('kri')}
          class={`px-4 py-3 rounded-2xl font-extrabold text-xs sm:text-sm whitespace-nowrap transition-all flex items-center gap-2 border-2 ${
            activeTab === 'kri'
        ...
        >
          ...
          <span>KRI Early Warning Radar (KRI)</span>
        </button>
        ...
        >
          <Award class="w-4 h-4" />
          <span>Risk Management Quiz 🎮</span>
        </button>
      ...
      <div class="bg-white rounded-3xl p-6 sm:p-8 border-2 border-slate-200 shadow-sm">
        {activeTab === 'rmf' && <RmfLifecycleSim />}
        {activeTab === 'kri' && <KriRadarSim />}
        {activeTab === 'quiz' && <RiskManagementQuiz />}
      </div>
